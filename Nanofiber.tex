...
The output field  are given by the Fourier transform of \erf{Eq::aout}, yielding \cite{le_kien_correlations_2008} 
...
The quantum effects stemming from the first term give rise to shot noise in the detected transmitted field.  
The second term represents that scattering into the guided mode, as described by the dyadic Green's function, Eq. (?).  
...
With the atoms trapped along the quasi-$H$ axis, the off-diagonal elements in \erf{Eq::GenHamiltonian} vanish, $\expt{ \hat{K}_{HV} } = \expt{ \polterm_{HV} } =0$, while $\expt{ \hat{K}_{HH} } \neq  \expt{ \hat{K}_{VV} }$.  
...
%	\end{align}
Shot noise determines the fundamental resolution of the polarimeter,.  The shot-noise variance $\Delta \mathcal{M}^2|_{SN} =  \dot{N}_L T$, for integration time $T$.  The minimum resolution for atom number detection using this dispersive measurement is thus, $\delta N_A \sim ( \chi_N^2 \dot{N}_L T)^{-1/2}$ \cite{smith_faraday_2003}.  
...
Because the $z$-component of the guided modes is $90^\circ$ out of phase with the transverse components, the quantization axis maximizing the atom-light coupling is specified by an angle in the transverse \emph{xy}-plane, $\qaxis = \cos \phi_q \mbf{e}_x + \sin \phi_q \mbf{e}_y$.  In \frf{fig:domega_magic12_q_NA2500_r1d8a} we plot $|\chi_{J_3}|^2$ as a function of $\phi_q$.  
Since measurement strength $\kappa =|\chi_{J_3}|^2 \dot{N}_L$  determines the amount of the spin squeezing in the absence of decoherence, \frf{fig:domega_magic12_q_NA2500_r1d8a} shows that the optimal choice of the quantization axis due solely to the coherent interaction is along the $x$-axis.  
As we will see in the next section, the optimal choice of quantization axis is modified due to the relative strength of coherent coupling and decoherence due to optimal pumping. 
