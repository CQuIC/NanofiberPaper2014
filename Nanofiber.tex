...
%The field has been expanded in the transverse mode functions,
%	\begin{align} \label{Eq::uin}
%		\mbf{u}_{\rm in}(\mbf{r}_\perp) = \sum_{p} c_{p} \mathbf{u}_{\fwd,p}(\br_\perp),
%	\end{align}
%with expansion coefficients satisfying $\sum_{p} |c_{p}|^2 = 1$.  
The effective mode area at the atom's position is determined from the total cycle-averaged power transported along the nanofiber, $P_{{\rm in},z} = (v_g/2\pi) \int d^2\br |\mathbf{E}^{(+)}_{\inp}(\br) |^2$, and the intensity at the atom, $I_{\rm in}(\mathbf{r}') = (c/2\pi) |\mathbf{E}^{(+)}_{\inp}(\br) |^2$,\comment{we'll discuss factors of $\pi$} via the relation \cite{domokos_quantum_2002},
