...
For detuning $\Delta$ large compared to the excited hyperfine splitting on the \comment{D1 or D2 lines}, the unit-oscillator scattering rate is $\gamma_s =(\sigma_0 /A_{\rm in})(\Gamma/2 \Delta)^2 \dot{N}_L $, with effective area determined by the probe at the atomic position, \erf{Eq::AreaIn}.  
In this limit, the rotation angle $\chiN = C^{(0)}_{j'} (\sigma_0/\Abir)(\Gamma/2\Delta)$, \erf{Eq::RotationAngle}, yields a shot noise-limited atom number resolution, 
	...
	\end{align}
\comment{where $ C^{(0)}_{j'}=\sum_{f,f'}C^{(0)}_{ff'}=2^{(j'-1/2)}/3  $ with $ j'=1/2,3/2 $ are the far-tuning irreducible rank-$0$ coefficients for the D1 and D2 line transitions. 
...
{\color{red} In general,} loss and decoherence due to the imperfection of atomic trapping and photon detection will degrade the measurement for long integration times and {\color{red} will} yield a worse atom number resolution than the minimum detectable atom number roughly estimated above~\cite{dawkins_dispersive_2011, zhang_collective_2012}. The experimental implementation of the dispersive atom number measurement reported by Dawkins \emph{et al.}~\cite{dawkins_dispersive_2011} implies a few tens of atom number resolution can be reached when $ 200\sim 1000 $ atoms are loaded at $ r'\approx 1.9a $ with the fiber radius $ a=250nm $. 
A similar scheme, recently employed by \emph{B\'{e}guin et al.} \cite{beguin_generation_2014} to perform nondestructive atom number measurements in a nanofiber geometry,  reported an improved atom number resolution of $\delta N_A = 8$ for $\expt{N_A}\sim2500$ atoms trapped at $ r'\approx 1.8a $ ($a=250nm$) by using the differential signal of two-color probes.  {\color{red}  The optical nanofiber provides a promising geometry in which to perform continuous atom number measurements at the level of a few atoms when coupled to guided light} in the dispersive regime. }
...
(b) Magnitude of the magic detunings, $| \tilde{\Delta}_{f}|/2\pi \equiv | \omega_0 - \magic{f} |/2\pi$, {\color{blue} at which the clock states are equally light-shifted and (c) the coupling strength, as measured by the polarization rotation angle on the Poincar\'{e} sphere, $\chi_{\tiny J_3}$ for an atom trapped in the $x$-$z$ plane, a distance $ r'\!_\perp=1.8a $ from the fiber surface.  This coupling strength varies as a function of the direction of the quantization axis that defines the clock states, \erf{Eq::QuantizationAxis}.  We plot $\chi_{\tiny J_3}$ as the direction of the clock state quantization axis is varied in the $x$-$y$ plane.}
...
Since the $z$-component of the guided modes is $90^\circ$ out-of-phase with the transverse components, the quantization axis maximizing the atom-light coupling is specified by an angle in the transverse $x$-$y$ plane, $\phi_\pi$,
...
\caption{Dependence of metrological squeezing on quantization axis, $\qaxis$, for $2500$ atoms trapped on the $x$-axis at a distance $ r'\!_\perp=1.8a$. In (b--d) $\qaxis$ is confined to the $x$-$y$ plane where the optimal peak squeezing occurs.
...
(a) OD/atom. Inset is the corresponding magic detuning in units of MHz.
(b) Optimal quantization axis orientation angle $\qaxis$ in the $x$-$y$ plane for different atom numbers. 
The line with $ 500 $ atoms is cut off when the squeezing effect is too weak to be observed ($ r'\!_\perp>2.3a $).
...
Eqs.~\eqref{Eq::gammaf} and~\eqref{Eq::gammaff} yield,
	\begin{align}
		...
		%&=\sum_{f_a,f_b,q}\big| \bra{f,0} \hat{W}_q^{f_bf_a} \ket{\tilde{f},0} \big|^2\\
