...
where $\chi_\eff$ is given in \erf{Eq::chieff}. As discussed in Sec. \ref{Sec::AtomNumberMeasurement}, the  operator $\hat{\mathcal{M}}$ describes an integrated homodyne measurement of the $X_L$-quadrature, given in \erf{Eq::Xquad}, 
	...
	\end{align}
where the input polarization mode plays the role of the local oscillator  \cite{vasilyev_quantum_2012,baragiola_three-dimensional_2014}. Quantum backaction generates squeezing when the projection noise is resolved above the inherent shotnoise of the light as characterized by the signal-to-noise ratio in the measurement fluctuations \cite{deutsch_quantum_2010, baragiola_three-dimensional_2014},
	\begin{align}
		R^2 \equiv \frac{\Delta \mathcal{M}^2_{\rm PN}}{ \Delta \mathcal{M}^2_{\rm SN} }.
	\end{align}  

For large atom numbers under consideration here, the collective atomic state is Gaussian--entirely described by collective means and covariances. The entangling interaction, \erf{Eq::FaradayHam}, preserves the Gaussian nature \cite{madsen_spin_2004}, and a measurement on the light yielding random result $\hat{X}_L^{\rm out} \rightarrow x_L$ projects the $J_3$-component of the mean spin \comment{$\expt{\hat{J}_3}\big|_{x_L} = $}. In the absence of decoherence the interaction is purely QND the conditional projection noise is squeezed according to \cite{}
	\begin{align} \label{Eq::IdealSqueezing}
		\big(\Delta J_3^\out \big)^2 = \big( \Delta J_3^\inp \big)^2 \frac{1}{1+R^2},
	\end{align}
with $R^2 = \kappa t \big( \Delta J_3^\inp \big){}^2$.  To the extent that the Gaussian approximation holds,  the collective spin can be squeezed indefinitely, even for minuscule measurement strength $\kappa$, by measuring for longer. The weak continuous measurement converges to a projective measurement in the long-time limit, and the collective state limits to a Dicke state of definite spin projection along $J_3$, with probabilities determined by the initial projection noise \cite{Stockton}. 


	%===================Optical Pumping=====================%
	\subsection{Optical pumping}

The situation described above where QND squeezing continues unhindered is not achievable, even in principle. The effective light-shift interaction, \erf{Eq::PropagatingElectricField}, inevitably contains an anti-Hermitian part to satisfy the Kramers-Kronig relationship. Physically, this corresponds to \emph{optical pumping}, whereby atoms throughout the ensemble becoming locally excited and then incoherently emitting photons into both guided and radiation modes.
	
Optical pumping gives rise to two processes, each of which affects spin squeezing differently \cite{madsen_spin_2004,norris_enhanced_2012}. First, \emph{loss} occurs when atoms are pumped out of the clock subspace to other magnetic sublevels. The effective atom number decreases, which shrinks the size of the collective spin but also reduces the projection noise. More damaging to metrological spin squeezing are \emph{spin flips}, where atoms are incoherently pumped within the pseudospin. The loss of coherence reduces the mean spin length, Spin flips inject noise into $\Delta J_z^2$.  In both processes, the loss of coherence reduces the mean spin length but for spin flips the increased projection noise competes with the QND interaction and limits squeezing.  \comment{(when atoms are pumped to states within the $F=4$ manifold they can affect birefringence, while atoms pumped to $F=3$ are invisible to the probe. Can we really neglect the possible birefringence from the $F=4$ atoms)}. 

Define characteristic photon scattering rate, which is a function of quantization axis. 

\subsubsection{Discussion of spin flip rate}
At the magic wavelength, spin flips are dominated by the incoherent pumping of atoms from $\ket{\uparrow}$ to $\ket{\downarrow}$ as a result of the large ground hyperfine splitting (9.2 GHz in $^{133}$Cs), as seen in \frf{}. Spin flip rate
	\begin{align}
...
\subsubsection{Master equation description (local)}
...
\subsubsection{Stochastic master equation (collective)}
