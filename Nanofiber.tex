...
with ``effective detuning" set by the magic-wavelength condition,
	\begin{align} 
		\frac{\Gamma}{4\Delta_{\eff}} = (b_4 - b_3) =  \sum_{F'}  \left( C^{(2)}_{J'F'4}\frac{\error{10}}{\Delta_{4F'}} -  C^{(2)}_{J'F'3}\frac{\error{6}}{ \Delta_{3F'} } \right)\frac{\Gamma}{4},
	\end{align}
and effective area given by
	\begin{equation}
		A_{\eff}^{-1} = \error{2}n_g \frac{ |\mathbf{e}_\pi \cdot \mathbf{u}_H(\br'_\perp) |^2 |\mathbf{u}_V(\br'_\perp)|^2 - |\mathbf{e}_\pi \cdot \mathbf{u}_V(\br'_\perp)|^2 |\mathbf{u}_H(\br'_\perp)|^2}{ |\mathbf{u}_H(\br'_\perp)|^2 + |\mathbf{u}_V(\br'_\perp)|^2 }.
	\end{equation}	
\comment{Check the sign here.}
The quantization axis that maximizes $\chi_{\eff}$ is that which maximizes $|A_{\eff}^{-1}|$. \comment{Since $\mbf{u}_H$ is orthogonal to $\mbf{u}_V$ at the atom position ($ \phi'=0,\pi $), the optimal quantization axis exists when either $|\mathbf{e}_\pi \cdot \mathbf{u}_H(\br'_\perp) |^2|\mathbf{u}_V(\br'_\perp)|^2$ or $|\mathbf{e}_\pi \cdot \mathbf{u}_V(\br'_\perp) |^2|\mathbf{u}_H(\br'_\perp)|^2$ is $ 0 $ and the other reaches the maximum.} One can show that 
	\begin{align}
		...
		A_{\eff}^{-1} \big|_{\rm opt} = & \error{2} n_g \frac{|\error{u_H^x}(\br'_\perp) |^2 |\mathbf{u}_V(\br'_\perp)|^2}{|\mathbf{u}_H(\br'_\perp)|^2 + |\mathbf{u}_V(\br'_\perp)|^2} \quad \text{for} \quad \mbf{e}_\pi = \mbf{e}_x.
	\end{align}
...
\begin{minipage}{.49\linewidth}
\centering
\subfloat[]{\label{fig:sigma0A_D1}\includegraphics[scale=0.45]{./Figs/sigma0A_D1}}
\end{minipage}
\caption{OD per atom as functions of atoms' radial position $ r\!_\perp $.}
\end{figure}
