...
The adjoint form of the SME in \erf{Eq::SME} describes the evolution of the collective operators in the Heisenberg picture. For a collective operator single-body operator $\hat{X} = \sum_n \hat{x}^{(n)}$ the evolution due to optical pumping $d\expt{ \hat{X}}|_{\rm op} = \sum_n \Tr [\mathcal{D}_n[\hat{\rho}]\hat{X} ] = \sum_n \Tr [\expt{\mathcal{D}_n\dg[\hat{x}^{(n)}]} ] $, where the map, which acts locally on atoms along the nanofiber, is given in \erf{Eq::OpticalPumpingMapSchr}. %Applying the map to the single-atom identity, $\mathcal{D}\dg[\hat{\mathbbm{1}}] = -\smallfrac{1}{2} \big( \Gamma_{\rm loss} \hat{\mathbbm{1}} + D_2\hat{\sigma}_z \big)$, and using the fact that $N_A = \sum_n \hat{\mathbbm{1}}^{(n)}$, 
	\begin{align}
