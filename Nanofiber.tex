...
Equation (\ref{Eq::GreensGuided}) is the central result from which we can calculate the dispersive response.  Consider a forward-propagating {\color{blue} ($b=+$)} input field in the guided modes with frequency $\omega_0$, positive-frequency amplitude $\Eamp$, and arbitrary polarization, $\mathbf{E}^{(+)}_{\inp}(\br) = \Eamp  \mathbf{u}_{\rm in}(\br_\perp) e^{i \beta_0 z}$ {\color{red} (Is there any confusion between the + for forward-propagating and the + for positive frequency component of the E-field? Along these lines, I changed the $b = \pm 1 \rightarrow b = \pm$. The digit 1 makes no sense there, since this is just a label for a mode, not a value.)}.   
...
For detuning $\Delta$ large compared to the excited hyperfine splitting on the \comment{D1 line}, the unit-oscillator scattering rate is $\gamma_s =(\sigma_0 /A_{\rm in})(\Gamma/2 \Delta)^2 \dot{N}_L $, with effective area determined by the probe at the atomic position, \erf{Eq::AreaIn}.  
...
(b) Magnitude of the magic detunings, $| \tilde{\Delta}_{f}|/2\pi \eqiov | \omega_0 - \magic{f} |/2\pi$, and (c) dimensionless rotation angle  $\chi_{\tiny J_3}$ for an atom trapped on the $x$-axis at $ r'\!_\perp=1.8a $ as the quantization axis, \erf{Eq::QuantizationAxis}, is varied in the $x$-$y$ plane. 
\comment{See text for details.}}\label{Fig::CouplingStrength}
\end{figure}
%=============================================
...
Using these magic detunings, in \frf{Fig::CouplingStrength}(c) we show the variation in $\chieff$ as a function of $\phi_\pi$.  
...
Restricting to the two-dimensional subspace associated with the clock states, the action on the $n^{th}$ atom is
	...
	\end{align}
Here, \comment{$\gamma_{f}$ is the total rate of photon scattering by atoms in the clock state $\ket{f,0}$, and $\gamma_{f \rightarrow \tilde{f} }$ are the rates of optical pumping between the clock states, $\ket{f,0} \rightarrow \ket{\tilde{f},0}$ (see Appendix~\ref{Appendix::OpticalPumping}). } 
...
Typical time evolution is shown in \frf{Fig::Squeezing_Dynamics} for $2500$ atoms trapped on the \emph{x}-axis at a distance $r'\!_\perp=1.8a$ from the center of the nanofiber, {\color{blue} where time is scaled} to the characteristic scattering rate, $\gamma_s \equiv \frac{\sigma_0}{A_{\rm in}}\frac{\Gamma^2}{4 \Delta_\eff^2} \dot{N}_L $.  We study the dynamics for two choices of quantization axis: (i) along the $x$-axis and (ii) along the numerically determined optimal axis.  Figure \ref{Fig::Squeezing_Dynamics}(a) shows the time evolution of the squeezing parameter.  {\color{blue} We achieve a maximum squeezing of} 4.7 dB when the clock states are chosen along the optimal axis; $\qangle^{\rm opt} \approx 86^\circ$ in \erf{Eq::QuantizationAxis}. 
...
The inset shows the decomposition at the optimal quantization axis of $ \Delta J_3^2 $ (red dashed) into the single-atom variance, $N_A (\Delta j_3^{(1)})^2$ (green) and the two-body covariance, $N_A(N_A-1)\expect{\Delta j_3^{(1)}\Delta j_3^{(2)}}$ (black), as given by \erf{Eq::VarianceDecomposition}..
...
We therefore set $ \expt{\hat{N}_C\hat{J}_3}-\expt{\hat{N}_A}\expt{\hat{J}_3}\approx 0 $ and $ \expt{\hat{N}_C^2} - \expt{\hat{N}_C}^2 \approx 0 $ and define $ N_C\equiv \expt{\hat{N}_C}$. 
...
\comment{Here, $\gamma_{f}$ is the total rate of photon scattering by atoms in the clock state $\ket{f,0}$, defined by
	\begin{equation}\label{Eq::gammaf}
		\gamma_{f}=- \frac{2}{\hbar} {\rm Im} \big[ \bra{f,0}\comment{ \hat{h}_{\rm eff}}\ket{f,0} \big] .
	\end{equation}
The effective non-Hermitian light-shift Hamiltonian that accounts for the imaginary part of the atomic polarizability is found by substitution of $\charpol \rightarrow -\frac{\sigma_0}{4\pi k_0}\frac{\Gamma/2}{\Delta_{ff'}+i\Gamma/2}$ in \erf{Eq::LightShiftHam}.  The rates of optical pumping between the clock states, $\ket{f,0} \rightarrow \ket{\tilde{f},0}$, are
	\begin{equation}\label{Eq::gammaff}
		\gamma_{f \rightarrow \tilde{f} } =\sum_{f_a,f_b,q}\big| \bra{f,0} \hat{W}_q^{f_bf_a} \ket{\tilde{f},0} \big|^2,
	\end{equation}
where $\hat{W}_\pi$ is the Lindblad jump-operator associated with absorption of the probe photon followed by spontaneous emission of a $q$-polarized photon ($q=0,\pm 1$ corresponding to $\pi$ and $\pm \sigma$ transitions)~\cite{deutsch_quantum_2010}.  

To calculate the rates, we define a characteristic photon scattering rate, $\gamma_s \equiv \frac{\Gamma\Omega^2}{4\Delta_{\rm eff}^2}= \frac{\sigma_0}{A_{\rm in}}\frac{\Gamma^2}{4 \Delta_\eff^2} \dot{N}_L $, to capture the input field intensity, where the Rabi frequency $ \Omega=2\bra{J}|d|\ket{J'}\mathcal{E}^{(+)}_{\rm in}/\hbar $ with reduced optical dipole matrix element $\bra{J}|d|\ket{J'}$ and the field amplitude $ \mathcal{E}^{(+)}_{\rm in}=|\mathbf{E}_{\rm in}^{(+)}(\br')| $.
Using the Wigner-Eckart theorem, Eqs.~\eqref{Eq::gammaf} and~\eqref{Eq::gammaff} yield,
	\begin{align}
		...
		&= \gamma_s \sum_{f'} \frac{\Delta_{\rm eff}^2}{\Delta_{ff'}^2}\sum_q \left| o_{jf}^{j'f'}C_{f'q}^{f0;1 q}\right|^2 \mathbf{e}_q^* \cdot (\mathbf{e}_{\rm in}\mathbf{e}_{\rm in}^* )\cdot \mathbf{e}_q, \\
		\gamma_{f \rightarrow \tilde{f}} &=\sum_{f_a,f_b,q}\big| \bra{f,0} \hat{W}_q^{f_bf_a} \ket{\tilde{f},0} \big|^2\\
		...
		&= \gamma_s \sum_{f'} \frac{\Delta_{\rm eff}^2}{\Delta_{ff'}^2}\sum_q \left| o_{j\tilde{f}}^{j'f'} o_{jf}^{j'f'}C_{f'q}^{\tilde{f}0;1 q}C_{f'q}^{f0;1q}\right|^2 \mathbf{e}_q^* \cdot (\mathbf{e}_{\rm in}\mathbf{e}_{\rm in}^* )\cdot \mathbf{e}_q,
	\end{align}
where $ \sigma (\Delta_{ff'} )  = \sigma_0 \Gamma^2/4\Delta^2_{ff'}$ is the the scattering cross section at the probe detuning, and $ \mathbf{e}_q $ and $ \mathbf{e}_{\rm in} $ are the unit vectors for the spherical basis and input field respectively. 
$ \hat{W}_q^{F_bF_a} = \sum_{f'}\frac{\Omega/2}{\Delta_{f'f_a}+i\Gamma/2}(\mathbf{e}_q^*\cdot\hat{\mathbf{D}}_{f_bf'} )(\mathbf{e}_{\rm in}\cdot \hat{\mathbf{D}}^\dagger_{f'f_a} ) $ are the jump operators for optical pumping between magnetic sublevels~\cite{deutsch_quantum_2010}.
...
Similarly, the ratio between $ \kappa $ and $ \gamma_s $ characterize the coupling strength between the atom ensembles and the light, which is independent of light intensity. Together with those rates discussed above, the spin squeezing dynamics can be fully studied independently of light intensity.}
