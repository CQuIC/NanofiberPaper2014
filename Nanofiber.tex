...
\begin{align}
\delta\phi &= -\frac{\Gamma_{1D}}{2\Delta},\\
\frac{\Gamma_{1D}}{\Gamma_0} &= \frac{6\pi c}{\omega}\frac{\sum_g\mathbf{d}_{eg}\cdot \mathrm{Im}\left[\mathbf{G}_g(\br',\br') \right]\cdot\mathbf{d}_{eg}^* }{\sum_g|\mathbf{d}_{eg}|^2} = 1+ \frac{3}{2} \frac{\mathrm{Im}\left[\sum_g\mathbf{d}_{eg}^*\cdot \mathbf{E}^{(R)}_g(\br') \right]}{\sum_g|\mathbf{d}_{eg}|^2 k^3}\nonumber\\
&=\frac{3\pi}{2}\left(\frac{c}{\omega}\right)^2\frac{c}{v_g} \frac{\sum_{g,m=\pm 1, f=\pm 1}|\mathbf{d}_{eg}\cdot \mathbf{u}^f_m(\br')|^2}{\sum_g|\mathbf{d}_{eg}|^2}\\
\mathbf{E}^{(R)}_g(\br') &= -\frac{1}{4\pi k^2} \mathbf{G}^{(R)}_g(\br',\br')\cdot \mathbf{d}_{eg},
\end{align}
the exact version of $ \delta\phi $ may include both the difference of the laser and atomic frequencies (detuning, $ \Delta $) and the sum of them for far-detuned energy levels. 
...
Discuss the homodyne detection and sensitivity to the atom number and operating frequency in the shot noise limit. 
