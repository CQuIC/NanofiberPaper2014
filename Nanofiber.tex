...
\affiliation{Center for Quantum Information and Control, University of Arizona, Tuscon, Arizona 87521, USA}
...
The developed formalism is used in \srf{Sec::QNDMeasurement} to study QND measurement of atoms based on polarization spectroscopy. 
...
Conclusions and an outlook for future research are summarized in \srf{Sec::Conclusion}.  
...
\end{align}
\end{subequations}
substituting into \erf{Eq::dsigma}, and making the usual Markov approximation gives an expression for the ground-excited state coherences,
\begin{align}
...
(b) Magnitude of the magic detunings, $|\Delta^m_{F}|/2\pi$, and (c) dimensionless rotation angle $\chieff$ for an atom trapped on the \emph{x}-axis at $ r'\!_\perp=1.8a $ as the quantization axis, \erf{Eq::QuantizationAxis}, is varied in the $xy$-plane. }\label{Fig::CouplingStrength}
\end{figure}
%=============================================
...
% Delta J3_2 decomposed into variance and covariance parts.
\begin{figure*}[t]
\includegraphics[scale=0.42]{./Figs/Jz2varcovar_qoptimal_NA2500_r1d8a}
\caption{Decomposition of $ \Delta J_3^2 $ into its variance and covariance parts for $2500$ atoms trapped on the \emph{x}-axis at distance $ r'\!_\perp=1.8a$. 
The optimal quantization axis is assumed. Values are normalized to $ N_A/4 $. 
Dashed line indicates $\Delta J_3^2$. 
Dotted dash line is the variance of one-body operators expressed as $N_A (\Delta j_3^{(1)})^2$.
Dotted line is the covariance of two-body operators, $N_A(N_A-1)\expect{\Delta j_3^{(1)}\Delta j_3^{(2)}}$, which has a steep negative correlation between pairwise psuedospins due to the measurement backaction and slowly recovers to zero due to atom loss in the clock state subspace. 
This figure can be used as an inset of Fig.~\ref{Fig::Squeezing_Dynamics}(c).
}\label{Fig::Jzvarcovar_qoptimal_NA2500_r1d8a}
\end{figure*}
...
First, the rate of squeezing is determined by the OD/atom $=\sigma_0 A_{\rm in}/A_{J_3}^2$ (equivalent to the ratio of the measurement strength to the scattering rate, $\kappa/\gamma_s$), which peaks when $\qaxis$ is along the $y$-axis, as seen in \frf{Fig::Squeezing_QuantizationAxis}(a). 
