...
&d \expt{\Delta \hat{J}_3^2}  = - \kappa \expt{\Delta \hat{J}_3^2}^2 dt \comment{- 2 \gamma_{33}} \expt{\Delta \hat{J}_3^2}dt + \frac{1}{4}\comment{\left(2\gamma_{33}-\gamma_{00} \right)} \tilde{N}_A dt + \comment{\left(\frac{\gamma_{30}}{2}-\gamma_{03}\right)} \expect{\hat{J}_3} dt    
\end{align}
...
\end{minipage}
\caption{ (a) $\Delta_m/2\pi$ as a function the atom's distance from the nanofiber for two choices of $\qaxis$, along the \emph{x}-axis and \emph{y}-axis. (b) Variation of $\Delta_m/2\pi$ at fixed $ r'\!_\perp=1.8a$ for different choices of quantization axis. \comment{Should be combined with other figures.}} %\label{Fig::D1_magicfreqs44_transition}
\end{figure}
...
However, in \frf{fig:xi_q_D1_NA2500_r1d8a} we see that when optical pumping is included, the optimal $\qaxis$ is rotated away from the \emph{y}-axis. 
...
The squeezing of $\varz$ is determined by the measurement strength, $\kappa/\gamma_s$, shown in \frf{fig:ximax_q_D1_xyplane_NA2500_r1d8a}, where we define a characteristic photon scattering rate $\gamma_s = (\sigma_0/\comment{A_{\rm in}})(\Gamma/2 \Delta_\eff)^2 \dot{N}_L$ that follows from \erf{Eq::chieff}. 
...
The relative rates of atom loss, of decoherence in $\varz$, and of spin flips that shorten $\expt{\hat{J}_x}$ are shown in \frf{fig:gamma34_q_D1_xyplane_NA2500_r1d8a_m44}. 

We investigate the dynamics in \frf{Fig::Squeezing_Dynamics} for several choices of $\qaxis$. 
In \frf{fig:xit_qXoptimal_r1d8a_NA2500} we investigate the dynamics of the squeezing parameter at the optimal quantization axis $\phi_Q=$ and compare to the idealized situation with no optical pumping where the dynamics results solely from the variations in $\kappa$ in \frf{fig:ximax_q_D1_xyplane_NA2500_r1d8a} and squeezing continues indefinitely (until the failure of the Gaussian approximation). Shown in \frf{fig:DeltaJz2t_qXoptimal_r1d8a_NA2500} is the decrease in the conditional variance $\varz$. While the decay of correlations, the primary limiting factor for peak achievable squeezing is the decay of the mean $\expt{\hat{J}_x}$ as optical pumping destroys the coherence between $\ket{\uparrow}$ and $\ket{\downarrow}$ within each atom. 
In \frf{fig:Jxt_qXoptimal_r1d8a_NA2500} we plot $\expt{\hat{J}_x}$ and the atom number $N_A$.  
The magic wavelength and effective detuning $\Delta_{J_3}$, given by \erf{Eq::SqueezingEffectiveDetuning}, are plotted in \frf{fig:domega_magic44_qXYoptimal_rp} and \frf{fig:domega_magic33_qXYoptimal_rp} as a function of $\phi_q$. 



As the atoms are trapped closer to the nanofiber surface, they experience higher guided-mode fields and the measurement strength increases exponentially, \frf{fig:xi_optimal_NA1000to5000_omega44}. 
