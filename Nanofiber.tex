...
\begin{align}
\dt{\hat{\sigma}_-} &= -i\omega_{eg} \hat{\sigma}_- + \sum_\mu \pi |g_\mu(\omega_{eg})|^2 \hat{\sigma}_z(t)\hat{\sigma}_-(t) - i\sum_\mu\int_0^\infty \mathrm{d}\omega g_\mu(\omega) \hat{a}^{in}_\mu(\omega)\hat{\sigma}_z(t) e^{-if\omega (t-t_0)}\\
&= (-i\omega_{eg}-\frac{\Gamma^g}{2})\hat{\sigma}_-(t) -i\sum_\mu\int_0^\infty \mathrm{d}\omega g_\mu(\omega) \hat{a}^{in}_\mu(\omega)\hat{\sigma}_z(t) e^{-if\omega (t-t_0)}, \label{eq:dsigma-t}
\end{align}
where we have used $\hat{\sigma}_z\hat{\sigma}_-=-\hat{\sigma}_- $, and have used the definition of the \textit{one dimensional} decay rate for the guided mode $\mu=(\beta_0,m,f)$ from Equ.~\ref{eq:Gammagmf}, $ \Gamma^g_{mf} $, and the total decay rate for the guided modes, $ \Gamma^g $, as
\begin{align}
...
\begin{align}
-i\omega \hat{\sigma}_-(\omega) &= \left[-i\omega_{eg}-\frac{\Gamma^g}{2} \right]\hat{\sigma}_-(\omega) +i\sum_\mu \sqrt{2\pi}g_\mu(\omega) \hat{a}^{in}_\mu(\omega)e^{i(1-f)\omega(t-t_0)}.
\end{align}
...
\hat{\sigma}_-(\omega) &= -\sum_\mu \frac{\sqrt{2\pi}g_\mu(\omega)e^{i(1-f)\omega(t-t_0)}}{\Delta+i\frac{\Gamma^g}{2}}\hat{a}^{in}_\mu(\omega),
\end{align}
...
\begin{align}
\hat{a}^{out}_\mu(\omega) &= \hat{a}^{in}_\mu (\omega) -i\sum_{\mu'}\frac{2\pi g_\mu^*(\omega)g_{\mu'}(\omega)e^{i(1-f')\omega(t-t_0)}}{\Delta+i\frac{\Gamma^g}{2}}\hat{a}^{in}_{\mu'}(\omega)=\sum_{\mu'}S^\mu_{\mu'}\hat{a}^{in}_{\mu'}(\omega),
\end{align}
where the scattering matrix elements
\begin{align}\label{eq:Smfmft}
S^\mu_{\mu'} &\equiv S^{mf}_{m'f'}=\delta_{mm'}\delta_{ff'}-i\frac{2\pi g_\mu^*(\omega)g_{\mu'}(\omega)e^{i(1-f')\omega(t-t_0)}}{\Delta+i\frac{\Gamma^g}{2}}.
\end{align}

For the $m=1,f=1$ mode, for instance, if we ignore the portions scattered to all the other modes, we will have 
\begin{align}
\hat{a}_{\mu}^{out}(\omega) &\approx \left[ 1- i\frac{\Gamma^g_{mf}}{\Delta+i\Gamma^g/2}\right] \hat{a}^{in}_\mu(\omega) \\
&= \left(\frac{\Delta+i(\Gamma^g/2-\Gamma^g_{mf})}{\Delta+i\frac{\Gamma^g}{2}} \right)\hat{a}^{in}_\mu(\omega).
\end{align}
\textcolor{blue}{Notice that, if we consider the decay into the radiative modes as well, the $\Gamma^g$ on the bottom of the equation above should be replaced into the total decay rate $\Gamma$ (see Ben's note on phase shift).}
...
\end{align}
\todo[inline]{To further generalize to the real atom case.}
...
with
\begin{align}
\hat{\mathbf{E}}^{(+)}(\br) &= \sum_{F=\pm 1} \hat{\mathbf{E}}^{(+)}_F(\br) = \sqrt{\frac{2\pi \hbar \omega_0}{v_g\tau}}\sum_{F=\pm 1} \left[E_H \mathbf{u}_{HF}(\br\!_\perp)\hat{a}_H + E_V \mathbf{u}_{VF}(\br\!_\perp)\hat{a}_V \right]e^{iF\beta z},
\end{align}
where $ E_{H/V} $ and $\mathbf{u}_{HF/VF}$ are the field amplitude and the polarization vector of the $H/V$ guided mode with propagation direction $F=\pm 1$. 

Each term of the effective Hamiltonian $\hat{H}_{\rm eff}^{FF'}$ in Equ.~\eqref{Eq::LightShiftHam} corresponds to the physics process with $F'$-propagating input field and $F$-propagating output field. For the case that only the forward-propagating fields are detected, the first two terms of the Hamiltonian yield the output we care about and the last two terms are losses. If the scattering among atoms can be ignored, then only the first term of the Hamiltonian is important. 
...
\begin{align}
\!\!\!\! \hat{H}_{\rm eff}^{FF'} &\!=\! \sum_{f} \hbar \chi_{0f}\! \sum_{f'}\frac{1}{1+\delta_{f'}/\Delta_f+i\gamma_{ff'}/2\Delta_f}\left[\hat{A}_0^{FF'}\hat{S}_0 \!+\! \hat{A}_1^{FF'}\hat{S}_1 \!+\! \hat{A}_2^{FF'}\hat{S}_2 \!+\!\hat{A}_3^{FF'}\hat{S}_3 \right]. 
\end{align}
...
\begin{align} \label{Eq::StokesHam}
	\hat{H}_{\rm coh}^{FF'} &= \sum_f \hbar \chi_0 \sum_{f'} \frac{1}{1+ \delta_{f'}/\Delta} \left[\hat{A}_0^{FF'}\hat{S}_0 +\hat{A}_1^{FF'}\hat{S}_1 +\hat{A}_2^{FF'}\hat{S}_2 +\hat{A}_3^{FF'}\hat{S}_3 \right]. 
\end{align}
...
\begin{align}
	\hat{A}_0^{FF'} = & C^{(0)}_{\rm j'f f'} \left[|E_H|^2\mathbf{u}_{HF}^*(\br'_\perp)\cdot\mathbf{u}_{HF'}(\br'_\perp)+ |E_V|^2\mathbf{u}_{VF}^*(\br'_\perp)\cdot\mathbf{u}_{VF'}(\br'_\perp) \right]\hat{I}_f \nonumber\\
	&+2iC^{(1)}_{\rm j'ff'}|E_H|^2\left[Fu_z^*(r_\perp^\prime)u_{r\!_\perp}(r_\perp^\prime)-F'u_{r\!_\perp}^*(r^\prime_\perp)u_z(r_\perp^\prime) \right]\hat{f}_\phi \nonumber\\
	&+ C^{(2)}_{\rm j'ff'} |E_H|^2 \left[Fu_z^*(r_\perp^\prime)u_{r\!_\perp}(r_\perp^\prime)+F'u_{r\!_\perp}^*(r^\prime_\perp)u_z(r_\perp^\prime) \right]\big(\hat{f}_{r\!_\perp}\hat{f}_z+\hat{f}_z\hat{f}_{r\!_\perp} \big)\nonumber\\
	&+ C^{(2)}_{\rm j'ff'}\!\sum_i \left[|E_H|^2u^{i^*}_{HF}(\!\br'_\perp\!)u^i_{HF'}(\!\br'_\perp\!)+ |E_V|^2u^{i^*}_{VF}(\!\br'_\perp\!)u^i_{VF'}(\!\br'_\perp\!) \right] \left(  \hat{f}_i^{2}\! -\! \frac{1}{3}\hat{\bf{f}}\! \cdot\!  \hat{\bf{f}} \right) \\
	% A1
	\hat{A}_1^{FF'} = & C^{(0)}_{\rm j'f f'} \left[|E_H|^2\mathbf{u}_{HF}^*(\br'_\perp)\cdot\mathbf{u}_{HF'}(\br'_\perp)- |E_V|^2\mathbf{u}_{VF}^*(\br'_\perp)\cdot\mathbf{u}_{VF'}(\br'_\perp) \right]\hat{I}_f \nonumber\\
	&+2iC^{(1)}_{\rm j'ff'} |E_H|^2\left[Fu_z^*(r_\perp^\prime)u_{r\!_\perp}(r_\perp^\prime)-F'u_{r\!_\perp}^*(r^\prime_\perp)u_z(r_\perp^\prime) \right]\hat{f}_\phi \nonumber\\
	&+ C^{(2)}_{\rm j'ff'} |E_H|^2 \left[Fu_z^*(r_\perp^\prime)u_{r\!_\perp}(r_\perp^\prime)+F'u_{r\!_\perp}^*(r^\prime_\perp)u_z(r_\perp^\prime) \right]\big(\hat{f}_{r\!_\perp}\hat{f}_z+\hat{f}_z\hat{f}_{r\!_\perp} \big)\nonumber\\
	&+ C^{(2)}_{\rm j'ff'}\!\sum_i \left[|E_H|^2u^{i^*}_{HF}(\!\br'_\perp\!)u^i_{HF'}(\!\br'_\perp\!)- |E_V|^2u^{i^*}_{VF}(\!\br'_\perp\!)u^i_{VF'}(\!\br'_\perp\!) \right] \left(  \hat{f}_i^{2}\! -\! \frac{1}{3}\hat{\bf{f}}\! \cdot\!  \hat{\bf{f}} \right) \\
	% A2
	\hat{A}_2^{FF'} = & 2C^{(1)}_{\rm j'ff'} \left[ 2\mathrm{Re} \left(E_H^*E_Vu_{r\!_\perp}^*(r_\perp^\prime)u_\phi(r_\perp^\prime) \right)\hat{f}_z \right.\nonumber\\
	&\quad\quad \left. -\left( FE_H^*E_Vu_\phi(r_\perp^\prime) u_z^*(r_\perp^\prime)+F'E_HE_V^*u_\phi^*(r^\prime_\perp)u_z(r_\perp^\prime) \right)\hat{f}_{r\!_\perp}\right] \nonumber\\
	&-i C^{(2)}_{\rm j'ff'} \left[ 2i\mathrm{Im} \left(E_H^*E_V u_{r\!_\perp}^*(r_\perp^\prime)u_\phi(r_\perp^\prime) \right)\left(\hat{f}_{r\!_\perp}\hat{f}_\phi +\hat{f}_\phi\hat{f}_{r\!_\perp} \right)\right.\nonumber\\
	&\quad \quad \left. + \left(FE_H^*E_V u_\phi(r_\perp^\prime) u_z^*(r_\perp^\prime) -F'E_HE_V^*u_\phi^*(r^\prime_\perp)u_z(r_\perp^\prime) \right)\big(\hat{f}_\phi\hat{f}_z+\hat{f}_z\hat{f}_\phi \big)\right] \\
	% A3
	\hat{A}_3^{FF'} = & -2iC^{(1)}_{\rm j'ff'} \left[ 2i\mathrm{Im} \left(E_HE_V^*u_{r\!_\perp}(r_\perp^\prime)u_\phi^*(r_\perp^\prime) \right)\hat{f}_z \right.\nonumber\\
	&\quad\quad \left. +\left( FE_H^*E_Vu_\phi(r_\perp^\prime) u_z^*(r_\perp^\prime)-F'E_HE_V^*u_\phi^*(r^\prime_\perp)u_z(r_\perp^\prime) \right)\hat{f}_{r\!_\perp}\right] \nonumber\\
	&+ C^{(2)}_{\rm j'ff'} \left[ 2\mathrm{Re} \left(E_HE_V^* u_{r\!_\perp}(r_\perp^\prime)u_\phi^*(r_\perp^\prime) \right)\left(\hat{f}_{r\!_\perp}\hat{f}_\phi +\hat{f}_\phi\hat{f}_{r\!_\perp} \right)\right.\nonumber\\
	&\quad \quad \left. + \left(FE_H^*E_V u_\phi(r_\perp^\prime) u_z^*(r_\perp^\prime) +F'E_HE_V^*u_\phi^*(r^\prime_\perp)u_z(r_\perp^\prime) \right)\big(\hat{f}_\phi\hat{f}_z+\hat{f}_z\hat{f}_\phi \big)\right].
\end{align}
\end{subequations}
