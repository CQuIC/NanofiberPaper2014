...
\frac{-4\pi\omega_0^2}{\omega_0^2\!\!-\!\omega^2(\beta)} \mathbf{u}_{\beta,p} (\br\!_\perp)\mathbf{u}^*_{\beta,p} 
...
In this limit, the rotation angle $\chiN = C^{(0)}_{j'} (\sigma_0/\Abir)(\Gamma/2\Delta)$, \erf{Eq::RotationAngle}, yields a shot noise-limited atom number resolution, 
	...
	\end{align}
where $ C^{(0)}_{j'}=\sum_{f,f'}C^{(0)}_{ff'}$ are the far-detuned, rank-$0$ coefficients \cite{deutsch_quantum_2010}. \comment{(If we are going to include details of D1 vs D2-line, then we should mention that the scattering cross section is different due to dependence on the wavelength, which is larger on the D1-line by 10\%).}
 
Using the parameters specified above, we find the minimum detectable atom number with a D1 probe to be $10\sim 20$ for atoms trapped at $ 1.5a\sim 2.0a $ away from the fiber axis. {\color{blue} As the fiber diameter increases in a reasonable range, the minimum detectable atom number will be increased by a small amount as a result of the stronger confinement of the light field as well as the larger $H$-to-$V$ mode intensity contrast at the atom position. \comment{Isn't a \emph{smaller} diameter better? The light is more tightly confined and the evanescent tail is bigger}. Using a D2 probe, the minimum detectable atom number will be decreased almost by a factor of 2 due to the stronger atom-light interaction. }
