...
\newcommand{\zrateA}{\Gamma_{1}}
\newcommand{\zrateB}{\Gamma_{2}}
...
For the generation of QND squeezing, each of the $N_A$ atoms is prepared in an equal superposition of $\ket{\uparrow}$ and $\ket{\downarrow}$ with respect to $\mbf{e}_\pi$, i.e. a SCS along $\hat{J}_x$, $ \ket{\Psi_A} = \big(\frac{\ket{\uparrow}+\ket{\downarrow}}{\sqrt{2}}\big) ^{\otimes N_A}$. The interaction, \erf{Eq::FaradayHam}, maps the atomic projection noise, $\Delta J_3^2$, onto the light, whose $S_3$-component is continuously monitored. A probe prepared along $\hat{S}_2$ with photon flux $\dot{N}_L$  implements a weak QND measurement of the collective atomic spin. The measurement strength,
	\begin{align} \label{Eq::MeasurementStrength}
		\kappa \equiv |\chi_\eff|^2 \dot{N}_L, 
	\end{align}
quantifies the rate at which conditional projection noise is reduced with $\chi_\eff$ given in \erf{Eq::chieff}. As discussed in Sec. \ref{Sec::AtomNumberMeasurement}, the  operator $\hat{\mathcal{M}}$ describes an integrated homodyne measurement of the anti-diagonal $X_L$-quadrature [\erf{Eq::Xquad}], 
...
Because of photon scattering into modes other than the forward guided mode, the atom-photon interaction will cause decoherence of the collective spin. The maximum achievable squeezing is determined by detailed balance of QND measurement and decoherence.  We can fully model this with a stochastic master equation (SME),
	...
	\end{align}
where $\hat{\rho}$ is the collective atomic state. The first two terms describe the QND measurement with strength $\kappa$, \erf{Eq::MeasurementStrength}. The conditional dynamics that result from the measurement are described by the superoperator
	\begin{align}
		\mathcal{H}[\hat{\rho}] = \hat{J}_z \hat{\rho} + \hat{\rho} \hat{J}_z - 2 \expt{\hat{J}_z} \hat{\rho},
	\end{align}
where $dW$ is a stochastic Weiner interval satisfying $dW^2 = dt$, and ``measurement backaction" is given by the collective Lindblad map, 
	\begin{align}
		\mathcal{L}[\hat{\rho}] = \hat{J}_z \hat{\rho} \hat{J}_z - \smallfrac{1}{2} \hat{J}_z^2 \hat{\rho} - \smallfrac{1}{2} \hat{\rho}  \hat{J}_z^2 .
	\end{align}
The final term in \erf{Eq::SME} describes the effect of optical pumping acting locally on the $n^{th}$ atom. Restricting to the two-dimensional subspace associated with the clock states for any given atom, the map for optical pumping follows from a full master equation description in the secular approximation \cite{deutsch_quantum_2010},
	\begin{align}
...
where $ \sigma (\Delta_{FF'} )  = \sigma_0 \Gamma^2/4\Delta^2_{FF'}$ is the the scattering cross section at the probe detuning.  When expressed in terms of Pauli operators on the pseudo-spin-1/2 of the clock states,
	...
	\end{align} 
There are two important features of this map that are not typical in a QND measurement for spin-1/2 particles.  Firstly, the map is {\em not} trace preserving to account for atoms optically pumped out of the clock subspace at rate $\lrate$ given in \erf{Eq::lrate}.  In addition, unbalanced optical pumping within the clock states can lead to a polarization of the mean along $\hat{J}_z$, different from the value found in a QND measurement.

Equations of motion for the collective moments follow from the SME, \erf{Eq::SME}, with local optical pumping described by the map in \erf{Eq::OpticalPumpingMapSchr}.  We present the equations without derivation here; details are provided in Appendix \ref{Appendix::OpticalPumping}. The conditionally squeezed variance satisifies 	
	\begin{align} \label{Eq::J3variance}
		\frac{d}{dt} \Delta J_z^2 %& = - \kappa \big( \Delta J_3^2 \big)^2 - 2\zrateB \Delta J_z^2 + \big( 2\zrateB -\lrate\big) \frac{N_A}{4} . \\
		& = - \kappa \big( \Delta J_z^2 \big)^2 - 2 \big( \lrate + \drate \big) \left( \Delta J_z^2 + \frac{N_A}{4} \right). 
	\end{align}
The optical pumping competes with the squeezing in two ways. Optical pumping reduces the number of atoms in the clock states according to $dN_A/dt =  - \lrate N_A$. Also, correlations between atoms that accumulate due to the QND measurement decay at rate 
	\begin{align}
		\drate = \gammaud + \gammadu.
	\end{align}
Optical pumping has an additional effect on metrological squeezing by shortening the macroscopic mean spin $ \expt{\hat{J}_x}$. This happens both through atom loss and through spin flips, $\ket{\uparrow_x} \rightarrow \ket{\downarrow_x}$, that occur at rate $\frate$ given in \erf{Eq::frate}. The mean spin satisfies
 	\begin{align}
		\frac{d}{dt} \expt{\hat{J}_x} =& - \big(\lrate + \frate ) \expt{\hat{J}_x}
	\end{align}
and decays exponentially. For no atom loss, $\lrate = 0$, the map in \erf{Eq::OpticalPumpingMapSchr} is trace-preserving, as would be the case for genuine spin-1/2 ground-state atoms such as $^{171}$Yb.  In this case \erf{Eq::J3variance} can be solved analytically \cite{baragiola_three-dimensional_2014}, and the conditional variance $\Delta J_z^2$ reaches a steady state where the QND squeezing equilibrates with the decoherence. In our situation, we solve the equation numerically.
...
\begin{minipage}{.49\linewidth}
\centering
\subfloat[]{\label{fig:xit_q_r1d8a_NA2500}\includegraphics[scale=0.45]{./Figs/xit_q_r1d8a_NA2500}}
\end{minipage}
\begin{minipage}{.49\linewidth}
\centering
\subfloat[]{\label{fig:xit_qoptimal_r1d8a_NA1000to5000}\includegraphics[scale=0.45]{./Figs/xit_qoptimal_r1d8a_NA1000to5000}}
\end{minipage}
...
The adjoint form of the SME in \erf{Eq::SME} describes the evolution of the collective operators in the Heisenberg picture. For a collective operator single-body operator $\hat{X} = \sum_n \hat{x}^{(n)}$ the evolution due to optical pumping $d\expt{ \hat{X}}|_{\rm op} = \sum_n \Tr [\mathcal{D}_n[\hat{\rho}]\hat{X} ] = \sum_n \Tr [\expt{\mathcal{D}_n\dg[\hat{x}^{(n)}]} ] $, where the map, which acts locally on atoms along the nanofiber, is given in \erf{Eq::OpticalPumpingMapSchr}. Applying the map to the single-atom operators, 
	\begin{align}
		\mathcal{D}\dg[\hat{\mathbbm{1}}] & = - \lrate \hat{\mathbbm{1}} - \prate \hat{\sigma}_z \\
		\mathcal{D}\dg[\hat{\sigma}_z] & = - \big( \prate - \drate \big) \hat{\mathbbm{1}} - \big( \lrate + \drate \big) \hat{\sigma}_z \label{Eq::zdecay} \\
...
The two-body correlations that form the basis for spin squeezing decay according to \cite{baragiola_three-dimensional_2014}
	...
	\end{align}
where the superscript refers to the $m^{th}$ and $n^{th}$ atoms.  Within the pseudospin of a single atom the decay is given by \erf{Eq::zdecay}.  The equations of motion for the moments of $\hat{J}_z$ are now found from the SME, \erf{Eq::SME}, 
	\begin{subequations} \label{Eq::J3MomentEquations}
	\begin{align} 
		d \expt{\hat{J}_z} =& \error{\pm} \sqrt{\kappa} \Delta J_z^2 \, dW  - \zrateA \smallfrac{N_A}{2} dt - \zrateB \expt{\hat{J}_z}dt,  \\
		...
		=& \error{\pm} 2\sqrt{\kappa} \expt{\hat{J}_z}\Delta J_3^2 \, dW -  2 \zrateB \expt{\hat{J}_z^2}dt - \big( \lrate - 2\zrateB + 4\zrateA\expt{\hat{J}_z}  \big) \smallfrac{N_A}{4} dt. 
	\end{align}
	\end{subequations}
The stochastic term in $d\expt{\hat{J}_z^2}$ was simplified using $\expt{J_z^3} = 3\expt{\hat{J}_z^2}\expt{\hat{J}_z}- 2\expt{\hat{J}_z}^3$, which follows from Gaussian statistics \cite{jacobs_straightforward_2006}. Finally, since the It\={o} calculus governing the stochastically evolving moments requires that differentials be taken to second order, the evolution of the variance follows from the relation $d \Delta J_3^2 = d \expt{\hat{J}_3^2} - 2 \expt{\hat{J}_3} d \expt{\hat{J}_3} - \big( d \expt{\hat{J}_3} \big)^2$. In the limit where $\expt{\hat{J}_z}\ll N_A$ the terms proportional to the stochastically fluctuating mean can be dropped, and the equation of motion for the conditional variance is given in \erf{Eq::J3variance}.
