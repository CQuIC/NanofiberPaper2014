...
\end{minipage}
\caption{(a) Energy level structure showing the two magic frequencies for probing the clock states on the D1-line of $^{133}$Cs. 
(b) Magic detunings, $\Delta_{F}/2\pi \equiv (\omega_{0} - \magic{F})/2\pi$, as a function of the atom's distance from the nanofiber for several quantization axes. 
(c) Magnitude of the magic detunings, $\Delta_{F}/2\pi$, and (d) dimensionless rotation angle $\chieff$ for an atom trapped on the \emph{x}-axis at $ r'\!_\perp=1.8a $ as the quantization axis, \erf{Eq::QuantizationAxis}, is varied in the $xy$-plane. }\label{Fig::CouplingStrength}
\end{figure}
%=============================================
...
Finally, in \frf{Fig::CouplingStrength}(d) we show the variation in $\chieff$ as a function of $\qaxis$.  
...
\end{minipage}
\caption{Dependence of metrological squeezing on quantization axis, $\qaxis$, for $2500$ atoms trapped on the \emph{x}-axis at a distance $ r'\!_\perp=1.8a$. In (b--d) $\qaxis$ is confined to the \emph{xy}-plane, where optimal peak squeezing occurs.
(a--b) Peak squeezing for varying $\qaxis$. 
(c) OD/atom.
(d) Rates of atom loss, $\gamma_{00}$, and depolarization, $\gamma_{11}$. 
...
With only $2500$ atoms at $r_\perp = 1.8a$, peak squeezing can be as large as \comment{4.7 dB} when the clock states are chosen along the optimal axis, $\qangle^{\rm opt} \approx 86^\circ$ in \erf{Eq::QuantizationAxis}. 
...
Second, from the fact that magic frequency, $\magic{4}$, lies nearly equidistant from $F'=3$ and $F'=4$, see \frf{Fig::CouplingStrength}(b), one might conclude that quantization along $y$-axis also provides more protection from decoherence. 
...
\end{minipage}
\caption{Squeezing as a function of trapping distance $r_\perp$ and initial atom number $N_A$. (a) OD/atom. (b) Optimal quantization axis. 
%The line is cut off when the squeezing effect is too weak to be observed ($ r'\!_\perp>2.3a $).
(c) Peak metrological squeezing at the optimal quantization axis.} \label{Fig::Squeezing_Distance}
\end{figure}
%================================
