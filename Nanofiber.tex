...
Since the $z$-component of the guided modes is $90^\circ$ out-of-phase with the transverse components, the quantization axis maximizing the atom-light coupling is specified by an angle in the transverse \emph{xy}-plane, 
	...
	\end{align}
The variation in (?) we plot $|\chieff|^2$ as a function of $\phi_\pi$.  
...
Scattered photons destroy the initial coherence between $\ket{\uparrow}$ and $\ket{\downarrow}$ within a single atom, thus depolarizing the collective mean spin $\expt{\jx}$. 
...
This would reveal only that it is small compared to $\NA$ and experiences deterministic polarization towards $\ket{\downarrow}$.)}

The choice of quantization axis $\qaxis$ along which the clock states are defined affects both the measurement strength and the relative rates of optical pumping. Figure \ref{fig:kappa_q_D1_xyplane_NA2500_r1d8a_m44} suggests an optimal choice for $\qaxis$ along the $x$-axis, where $\chieff$ reaches a maximum for a probe detuned close to one of $F'=3$ and $F'=4$. 

In \frf{} the effective OD/atom is shown the quantization axis varied in the $xy$-plane with a maximum value of OD/atom $= $ for quantization along the $y$-axis.   
...
\end{minipage}
\caption{Dependence of metrological squeezing on quantization axis, $\qaxis$, for $2500$ atoms trapped on the \emph{x}-axis at a distance $ r'\!_\perp=1.8a$. 
\protect\subref{fig:xi_q_D1_NA2500_r1d8a} Peak squeezing for varying $\qaxis$. 
In figures (b--d) $\qaxis$ is confined to the \emph{xy}-plane where optimal peak squeezing occurs.
\protect\subref{fig:ximax_q_D1_xyplane_NA2500_r1d8a} Peak squeezing. 
\protect\subref{fig:OD_q_xyplane_r1d8a_m44} OD/atom.
\protect\subref{fig:gamma34togammas_q_D1_xyplane_NA2500_r1d8a_m44} Optical pumping rate $\gamma/\gamma_s$. 
}\label{Fig::Squeezing_QuantizationAxis}
\end{figure}
%================
