...
\end{minipage}
\caption{Dependence of light-matter coupling on choice of quantization axis, \erf{Eq::QuantizationAxis}, for an atom located on the \emph{x}-axis at distance $ r'\!_\perp=1.8a$. (a) Magic detuning, $\Delta_m/2\pi \equiv (\omega_{0}-\omega_{44'})/2\pi $, as a function of the atom's distance from the nanofiber for two choices of $\qaxis$, along the \emph{x} and \emph{y}-axes. (b) $\Delta_m/2\pi$ as a function of $\phi_Q$. }\label{Fig::CouplingStrength}
\end{figure}
%=============================================
...
\change{	\begin{align} \label{Eq::OpticalPumpingMapSchr}
	...
	\end{align} 
}\comment{(write in terms of the useful rates. Below, notations like $ \Delta_{\uparrow/\downarrow} $ seem confusing. Maybe we also want to keep definitions like $ \tilde{\gamma}_\uparrow =\gamma_\uparrow - \gammadu $ with proper physics interpretation to give alternative simpler expressions for some of our equations? ).} 
Here $\Delta_\uparrow \equiv \gamma_\uparrow - \big(\gammauu + \gammadu \big)$ is the total rate of population change for the state $\ket{\uparrow}$ and similarly for $\Delta_\downarrow$. There are two important features of this map that are not typical in a QND measurement for spin-1/2 particles.  Firstly, the map is {\em not} trace preserving, $\Delta_\uparrow + \Delta_\downarrow \neq 0$, to account for atoms pumped out of the clock states. In addition, since $\Delta_\uparrow \neq \Delta_\downarrow$, optical pumping will polarize the mean $\expt{\hat{J}_z}$, different from the value found in a QND measurement.

Equations of motion for the collective moments follow from the SME, \erf{Eq::SME}, with local optical pumping described by the map in \erf{Eq::OpticalPumpingMapSchr}.  We present the equations here with details of the derivations provided in Appendix \ref{Appendix::OpticalPumping}. The optical pumping competes with the squeezing in two ways. First, optical pumping reduces the number of atoms in the clock states according to $dN_A/dt \approx - \lrate N_A$ [see \erf{Eq::Idecay}]. Second, when an atom returns to the clock states after incoherently scattering a photon, the inter-atomic negative correlations that accumulate from the QND measurement decay at rate 
	\begin{align}
...
with rates	
	\begin{align}
		...
		\frate & =  \frac{\gammauu + \gammadd + \drate}{2}. \label{Eq::frate}
	\end{align}
