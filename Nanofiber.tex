...
Continuous monitoring of the output $S_3$-component implements a homodyne measurement of the $X_L$-quadrature, where the input polarization mode plays the role of the local oscillator.  $\hat{\mathcal{M}} = \int_0^t dt' \hat{S}^{\rm out}_3(t')$. In the absence of atom and photon loss, i.e. the interaction is purely QND, the integrated measurement operator for an input probe pulse of duration $t$ is \cite{hammerer_quantum_2010}
	...
	\end{align}
where the operators in \erf{Eq::OutputS3} are integrated over the pulse duration $t$, $\hat{O} = \int_0^t dt' \hat{O}(t')$. The fluctuations in the integrated polarimetry signal are found using \erf{Eq::XPcommutator}, 
	\begin{align}
...
and an effective area given by
	\begin{align}
		...
		& \change{ = n^2_g A_\inp \Big[ |\mathbf{e}_\pi \cdot \mathbf{u}_H(\br'_\perp) |^2 |\mathbf{u}_V(\br'_\perp)|^2 - |\mathbf{e}_\pi \cdot \mathbf{u}_V(\br'_\perp)|^2 |\mathbf{u}_H(\br'_\perp)|^2 \Big] /\error{2}. }
	\end{align}	
...
\begin{minipage}{.49\linewidth}
\centering
\subfloat[]{\label{fig:kappa_q_D1_xyplane_NA2500_r2a_m44}\includegraphics[scale=0.45]{./Figs/kappa_q_D1_xyplane_NA2500_r2a_m44}}
\end{minipage}
\begin{minipage}{.49\linewidth}
\centering
\subfloat[]{\label{fig:OD_q_xyplane_r2a_m44}\includegraphics[scale=0.45]{./Figs/OD_q_xyplane_r2a_m44}}
\end{minipage}
\caption{Study of squeezing parameters with rotating quantization axis with $2500$ atoms positioned at $ r'\!_\perp=2a $. Subfig.~\protect\subref{fig:xi_q_D1_NA2500_r2a} shows the squeezing parameter $ \xi^2 $ as the quantization axis rotates in the 3D space. Subfig~\protect\subref{fig:ximax_q_D1_xyplane_NA2500_r2a} gives the squeezing parameter when the quantization axis rotates on the $ xy $-plane. Subfig.~\protect\subref{fig:kappa_q_D1_xyplane_NA2500_r2a_m44} is $ \kappa $ (in arbitrary units) as a function of the orientation of the quantization axis rotating in the $ xy $-plane. Subfig.~\protect\subref{fig:OD_q_xyplane_r2a_m44} shows the $ \mathrm{OD}/{\mathrm{atom}} $ when the quantization axis is rotating in the $ xy $-plane.  }\label{fig:squeezing_q_D1_NA2500_r2a}
\end{figure}
