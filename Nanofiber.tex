...
We restrict here to the quasilinear modes, $p =\{H,V\}$, of a single HE$_{11}$ guided mode at frequency $\omega_0$, whose form is given explicitly in \erf{Eq::QuasilinearModes}.  In typical experimental configurations, two one-dimensional arrays of atoms are trapped on either side of the nanofiber, see Fig. \ref{}. The plane established by the atoms defines the quasi-$H$ ($\phi = 0$) and quasi-$V$ ($\phi = \pi/2$) axes. In the nanofiber region, the $H$-mode is purely $x$-polarized at $\phi = \pm \pi/2$ and the $V$-mode is purely $y$-polarized at $\phi = \{0,\pi\}$.  At other azimuthal angles the electric field is generally rotating along an ellipse.  The atoms trapped along the $x$-axis at $\phi'=0$ experience $H$ and $V$ fields,
	...
	\end{align}
where the real-valued functions $u_\alpha(r_\perp)$, given in \erf{Eq::ProfileFunctions}, depend only on the radial coordinate.  The $V$-mode is linearly polarized, while the $H$-mode is elliptically polarized due to the out-of-phase component along the propagation direction. 
...
Considering here only the forward-propagating guided modes $(f=+)$, we  drop the $f$ index.  The propagating electric field, \erf{Eq::PropagatingElectricField}, interacts with the trapped atoms via the dispersive light-shift Hamiltonian~\cite{deutsch_quantum_2010,kien_dynamical_2013},
	...
	\end{equation}
Here, $\poltens(\br_n)$ is the atomic tensor polarizability, given in \erf{Eq::PolarizabilityOperator}, for the $n^{th}$ atom trapped near the nanofiber surface at position $\mathbf{r}_n$.    Note, the atoms are typically trapped in two chains, at azimuthal angles $\phi=0,\pi$. The fields experienced by these two chains is equal in magnitude, but not in polarization.  This does not affect the QND measurement under consideration here because ...  We thus treat all atoms equivalently, with a total number of trapped atoms $N_A$.
...
\begin{align} \label{Eq::ScatteringSolution}
		\hat{a}^{\rm out}_{p}(t) = \hat{a}^{\rm in}_{p}(t) + i N_A \frac{2\pi \omega_0}{v_g} \sum_{p'} \mathbf{u}^*_p(\br') \cdot \poltens \cdot  \mathbf{u}_p(\br')  \hat{a}^{\rm in}_{p'}(t)
	\end{align} 
where have neglected any effects of retardation over the propagation distance of the nanofiber so that all $N_A$ atoms add equally to the signal (for details see \cite{BenDissertation}). The quantum effects stemming from the first term give rise to shot noise in the detected transmitted field.  The second term represents that scattering into the guided mode, as described by the dyadic Green's function, Eq. (?).  
...
%The trapped atoms are distributed at a fixed distance $r_\perp$ above and below the nanofiber, $N_A = N_0 + N_\pi$, where $N_{\phi}$ is the number of atoms at azimuthal angle $\phi$. For weak scattering, the collective atom-light Hamiltonian is
	\begin{align}  
		\hat{H}_{LS} 	= - 2 \pi \hbar k_0 n_g N_{A} \Big\{ &\big[ \hat{K}_{HH}+\hat{K}_{VV} \big] \hat{S}_0(t) +  \big[\hat{K}_{HH} - \hat{K}_{VV} \big] \hat{S}_1(t) \nonumber \\
+ &\big[\hat{K}_{HV} + \hat{K}_{VH} \big] \hat{S}_2(t) + i  \big[ \hat{K}_{HV}-\hat{K}_{VH} \big]\hat{S}_3(t) \Big\}, \label{Eq::GenHamiltonian} 
	\end{align}
The atomic couplings to the $\{H,V\}$ modes,
	\begin{align} 
		\hat{K}_{p p'}& \equiv |\mathbf{u}^*_p(\br')||\mathbf{u}_{p'}(\br')| \, \hat{\alpha}_{p p'}, 
	\end{align}
are formed by the quantum mechanical analog of the polarizability components from \erf{Eq::PolarizabilityElementsClassical}, $\hat{\alpha}_{p p'} = \mathbf{e}^*_{p'}  \cdot \poltens \cdot \mathbf{e}_{p}$, weighted by the transverse mode functions at the atomic position.
...
For using the D1-line of $^{133}$Cs atoms as the probe light, there are two magic frequency solutions, $ \omega_{44} $ and $ \omega_{33} $, as shown in \frf{fig:D1_magicfreqs44_transition} relative to the clock-state sublevels.
...
The magic detunings as a function of the direction of quantization axis, $ \phi_q $, on the transverse \emph{xy}-plane is shown in \frf{fig:domega_magic12_q_NA2500_r1d8a} for atoms trapped at typical experimentally-realizable \cite{} distance of $r_\perp'=1.8a$ from the center of the nanofiber. 
...
In \frf{fig:domega_magic12_q_NA2500_r1d8a} we plot the measurement strength, $ \kappa=|\chi_{J_3}|^2\dot{N}_L $, as a function of $\phi_Q$. 
Since $ \kappa $ indicates the spin squeezing effect without considering decoherence, \frf{fig:domega_magic12_q_NA2500_r1d8a} implies that the optimal choice of the quantization axis may be along the \emph{x}-axis as a result of the anisotropy of $ \chi_{J_3} $ which comes from both the internal structure of the atoms and the property of the nanofiber waveguide and the guided fields. 
