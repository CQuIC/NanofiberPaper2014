...
Dawkins {\em et al.} \cite{dawkins_dispersive_2011} used this interaction to make a dispersive measurement of $N_A$ by launching linearly polarized light at 45$^\circ$ to the quasi-$H$ axis; i.e. $\mbf{u}_\inp = (\mbf{u}_H + \mbf{u}_V )/\sqrt{2}$ in \erf{Eq::uin}.  From the perspective of the Stokes vector components, \erf{Eq::StokesComponents}, the propagating input field, $\langle \hat{S}_2^{\rm in}(t)\rangle  = \dot{N}_L/2$, rotates around the $S_1$-axis on the Poincar\'{e} sphere through an angle $\chibir$.  The polarization rotation is detected by measuring the output $S_3$-Stokes component, whose evolution follows from the scattering solution in \erf{Eq::ScatteringSolution}; $\hat{S}_3^{\rm out}(t) = \hat{S}_3^{\rm in}(t) + \chibir N_A \hat{S}_2^{\rm in}(t)$ \cite{hammerer_quantum_2010}.  When the interaction is purely QND, i.e. in the absence of atom or photon loss, an input probe pulse of duration $t$ evolves according to \cite{hammerer_quantum_2010}
	\begin{equation} \label{Eq::OutputS3}
		\hat{S}_3^{\rm out} = \hat{S}_3^{\rm in} \error{-} \chibir N_A \hat{S}_2^{\rm in},
	\end{equation}
where the unitless Stokes operators in \erf{Eq::OutputS3} are integrated over the pulse duration $t$, $\hat{S}_i = \int_0^t dt' \hat{S}_i(t')$. Using \erf{Eq::StokesCommutation}, we find the fluctuations in the integrated polarimetry signal \cite{baragiola_three-dimensional_2014}, 
...
The same birefringent interaction, \erf{Eq::GenHamiltonian}, can be utilized in a QND measurement to squeeze the projection noise of the collective atomic spin.  In particular, we consider squeezing of the uncertainty associated with the ``clock" transition of cesium, which to first order is insensitive to external magnetic fields.  The clock states, $\ket{\uparrow} = \ket{6S_{1/2}, F=4, M=0}$ and $\ket{\downarrow} \equiv \ket{6S_{1/2}, F=3, M=0}$, define a pseudospin within each atom, $\hat{\sigma}_z \equiv \op{\uparrow}{\uparrow} - \op{\downarrow}{\downarrow}$.  The quantum uncertainty in the collective pseudo-spin,
	\begin{align}
		\hat{J}_3 = \frac{1}{2} \sum_{i=1}^{N_A} \hat{\sigma}_z(\mbf{r}_i),
	\end{align}
...
At the magic wavelength the effective rotation angle $\chi_{\eff}$ can then be written
	\begin{align}
		\chi_{\eff} %& = \frac{\sigma_0}{A_\eff} 2\Delta b \\
			& =\frac{\sigma_0}{A_{\eff}} \frac{\Gamma}{4\Delta_{\eff}}
	\end{align}
with ``effective detuning" set by the magic-wavelength condition,
	\begin{align} 
		\frac{\Gamma}{4\Delta_{\eff}} = \error{2}(b_4 - b_3) =  \sum_{F'}  \left( C^{(2)}_{J'F'5}\frac{\error{20}\Gamma}{4\Delta_{4F'}} -  C^{(2)}_{J'F'3}\frac{\error{12}\Gamma}{ \Delta_{3F'} } \right),
	\end{align}
and effective area given by
	\begin{equation}
	...
	\end{equation}	
The quantization axis that maximizes $\chi_{\eff}$ is that which minimizes $A_{\eff}$. Defining an angle $\theta$ between the orthogonal linear components of $\mbf{u}_H$ and $\mbf{u}_V$, the projection onto the quantization axis gives $|\mathbf{e}_\pi \cdot \mathbf{u}_H(\br'_\perp) |^2 = \cos^2 \theta |\mathbf{u}_H(\br'_\perp) |^2$ and $|\mathbf{e}_\pi \cdot \mathbf{u}_V(\br'_\perp) |^2 = \sin^2 \theta |\mathbf{u}_V(\br'_\perp) |^2$.
	\begin{align}
		 %\cos^2 \theta |\mathbf{u}_H(\br'_\perp) |^2 |\mathbf{u}_V(\br'_\perp)|^2 - \sin^2 \theta | \mathbf{u}_V(\br'_\perp)|^2 |\mathbf{u}_H(\br'_\perp)|^2 = & \cos 2 \theta |\mathbf{u}_H(\br'_\perp) |^2 |\mathbf{u}_V(\br'_\perp)|^2 \\
		A_{\eff}^{-1} \big|_{\rm opt} = & n_g \frac{|\mathbf{u}_H(\br'_\perp) |^2 |\mathbf{u}_V(\br'_\perp)|^2}{|\mathbf{u}_H(\br'_\perp)|^2 + |\mathbf{u}_V(\br'_\perp)|^2} \quad \text{for} \quad \mbf{e}_\pi = \mbf{e}_x.
	\end{align}
