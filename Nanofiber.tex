...
In experimental situations, loss and decoherence due to the imperfection of atomic trapping and photon detection will degrade the measurement for long integration times and could yield a worse atom number resolution than the minimum detectable atom number roughly estimated above~\cite{dawkins_dispersive_2011, zhang_collective_2012}. 
The experimental implementation of the dispersive atom number measurement reported by \emph{Dawkins et al.}~\cite{dawkins_dispersive_2011} implies a few tens of atom number resolution can be reached when $ 200\sim 1000 $ atoms are loaded at $ r'\approx 1.9a $ with the fiber radius $ a=250nm $. 
...
It has been promising to realize continuous atom number measurements at the few atom level in the dispersive regime. }
...
(b) Magnitude of the magic detunings, $| \tilde{\Delta}_{f}|/2\pi \equiv | \omega_0 - \magic{f} |/2\pi$, and (c) dimensionless rotation angle  $\chi_{\tiny J_3}$ for an atom trapped on the $x$-axis at $ r'\!_\perp=1.8a $ as the quantization axis, \erf{Eq::QuantizationAxis}, is varied in the $x$-$y$ plane. 
