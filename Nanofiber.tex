...
\todo[inline]{ Need to be more careful about the $ e^{i\omega(t-t_0)} $ phase factor and its physics 
interpretation...}
...
\end{minipage}
\caption{Hyperfine level structure of the $ D_1 $ line transitions involving the clock states in $ 6S_{1/2} $ ground levels, and their transition probabilities (numbers on the transition lines). Subfig.~\ref{fig:D1line_F4} shows the transitions only involving the $ F=4,m=0 $ manifold. Subfig.~\ref{fig:D1line_F3} shows the transitions only involving the $ F=3,m=0 $ manifold. }
\label{fig:D1line}
\end{figure}
...
\right\} . 
\end{align}
For the $ D_1 $ line transition, for example, there are two magic wavelengths which close to the $ F=3 
\leftrightarrow F'=3 $ and $ F=4\leftrightarrow F'=4 $ transitions, respectively. Since the energy splitting 
between the $ F=3 $ and $ F=4 $ ground states is far large than the energy splitting for the excited states 
$ F'=3 $ and $ F'=4 $, we can ignore the coupling terms related to the far detuned ground state. That is to 
say, if we use the magic frequency, $ \omega_3^{\rm magic} $, which close to the $ F=3\leftrightarrow 
F'=3 $ 
transition, we would have
\begin{align}
\chi_{\rm eff} (\omega_3^{\rm magic}) &= - 8\pi k_0^2 \tr \left\{ \mathrm{Im}\left[ 
\mathbf{G}^*_{HH}(\br^\prime\!_\perp,\br^\prime\!_\perp) \right] \cdot  \boldsymbol{\alpha}^{30;30}
\right\}\\
...
\frac{|o^{J'F'}_{J3} C^{3 0;1q}_{F' q}|^2 }{\Delta_{3F'}+i\Gamma/2}  \right),
\end{align}
otherwise, for the magic frequency close to the $F=4\leftrightarrow F'=4  $ transition, $ \omega_4^{\rm 
magic} $,
\begin{align}
\chi_{\rm eff} (\omega_4^{\rm magic}) &=  8\pi k_0^2 \tr \left\{ \mathrm{Im}\left[ 
\mathbf{G}^*_{HH}(\br^\prime\!_\perp,\br^\prime\!_\perp) \right] \cdot  \boldsymbol{\alpha}^{40;40}
\right\}\\
&=  \sigma_0 n_g \frac{\Gamma}{2} \sum_q |\mathbf{e}_q\cdot 
\mathbf{u}_H^*(r^\prime\!_\perp,\phi')|^2 \sum_{F'}  \left(  
\frac{|o^{J'F'}_{J4} C^{4 0;1q}_{F' q}|^2 }{\Delta_{4F'}+i\Gamma/2}  \right).
\end{align}
The angle change on the Poincare sphere using the probe light at the magic frequency $ \omega_F^{\rm 
magic} $ can be given by
\begin{align}
\varphi_{_N} (\omega_F^{\rm magic}) &= N_A \chi_{e\!f\!f} (\omega_F^{\rm magic})\\
&= (-1)^F  N_A \sigma_0 n_g\frac{\Gamma}{2} \sum_q |\mathbf{e}_q\cdot 
\mathbf{u}_H^*(r^\prime\!_\perp,\phi')|^2 
\sum_{F'}  \left(  
\frac{|o^{J'F'}_{JF} C^{F 0;1q}_{F' q}|^2 }{\Delta_{FF'}+i\Gamma/2}  \right).
\end{align} 
...
\begin{align}
\sigma(\Delta_{FF'}) &=\frac{\sigma_0}{1+\frac{4\Delta^2_{FF'}}{\Gamma^2}}\approx \sigma_0 
\frac{\Gamma^2}{4\Delta^2_{FF'}} \quad \text{(far detuning.)}
\end{align}
Hence, we can rewrite 
\begin{align}
\Delta P_{SN} (\omega_F^{\rm magic}) &=\frac{P_0\Gamma}{2}\sqrt{\frac{ \sigma_0 }{2A_{in}}}\sum_{F'} 
\frac{1}{\Delta_{FF'}},\\
\Delta P_S (\omega_F^{\rm magic}) &= P_0  \varphi_{_N} (\omega_F^{\rm magic}) \nonumber\\
&=  (-1)^F  P_0 N_A 
...
\sum_{F'}  \left(  
\frac{|o^{J'F'}_{JF} C^{F 0;1q}_{F' q}|^2 }{\Delta_{FF'}+i\Gamma/2}  \right)\\
&\approx  (-1)^F  \frac{P_0 \Gamma}{2} N_A \sigma_0 n_g 
...
P_s = \Delta P_{SN} $ as
\begin{align}
N_A^{min}(\omega_F^{\rm magic}) &= \sqrt{\frac{(-1)^F}{2A_{in}\sigma_0}} 
\frac{\sum_{F'}\frac{1}{\Delta_{FF'}}}{ n_g 
\sum_q |\mathbf{e}_q\cdot \mathbf{u}_H^*(r^\prime\!_\perp,\phi')|^2 
\sum_{F'} \frac{|o^{J'F'}_{JF} C^{F 0;1q}_{F' q}|^2 }{\Delta_{FF'}}   }
\end{align}
