...
%	\end{align} 
	\begin{align} \label{Eq::aout}
		\hat{a}^{\out}_{bp}(\omega) = \hat{a}^{\inp}_{bp}(\omega) &- 2\pi i\sum_{b',p'} 
\sum_{e,g,g'}\!\hat{\sigma}_{gg'}\frac{ g_{\mu,e,g}^* g_{\mu',e,g'}}{ \omega-\omega_{eg} + i \Gamma_{e}/2 }\hat{a}^{\inp}_{b' p'}(\omega) \\
&- 2\pi i\sum_{m,p} \sum_{e,g,g'} \int^{kn_2}_{-kn_2} d\beta \,\hat{\sigma}_{gg'}\frac{ g_{\mu,e,g}^* g_{\nu',e,g'}}{ \omega-\omega_{eg} + i \Gamma_{e}/2 }\hat{a}^{\inp}_{bp}(\omega, \beta).\nn
	\end{align} 
...
{\color{blue} We give in this Appendix an explicit expressions for the photon scattering rates used in Sec.~\ref{Sec::QNDMeasurement} following the formalism given in~\cite{deutsch_quantum_2010}.  The} total rate of photon scattering by atoms in the clock state $\ket{f,0}$ is
...
\end{align}
as follows from \erf{Eq::LightShiftHam}, where here $\charpol = -\frac{\sigma_0}{4\pi k_0}\frac{1}{\Delta_{ff'}+i\Gamma/2}$ {\color{blue} is complex polarizability} and the irreducible tensor operator $ \hat{\tensor{\mbf{A}}}(f,f') $ is given in \erf{Eq::PolarizabilityIrrep}.
...
Each jump operator $\hat{W}_q^{f_bf_a}$ is associated with absorption of the probe photon polarized along $ \mathbf{e}_{\rm in} $ followed by spontaneous emission of a photon with polarization $ \mathbf{e}_q $, where $q=0,\pm 1$ labels spherical basis elements for $\pi$ or $\pm \sigma$ transitions.  

{\color{blue} To find} the dependence of the input field intensity, we define a characteristic photon scattering rate, $\gamma_s \equiv \frac{\Gamma\Omega^2}{4\Delta_{\rm eff}^2}= \frac{\sigma_0}{A_{\rm in}}\frac{\Gamma^2}{4 \Delta_\eff^2} \dot{N}_L $, where the Rabi frequency $ \Omega=2\bra{J}|d|\ket{J'}\mathcal{E}^{(+)}_{\rm in}/\hbar $ with reduced optical dipole matrix element $\bra{J}|d|\ket{J'}$ and the field amplitude $ \mathcal{E}^{(+)}_{\rm in}=|\mathbf{E}_{\rm in}^{(+)}(\br')| $.
...
{\color{blue}  $ C_{f'q}^{f0;1 q}=\Braket{f'q}{f0;1q}$ are the Clebsch-Gordan coefficients and }
\begin{equation}
\left|o_{jf}^{j'f'}\right|^2=(2j'+1)(2f+2) \left\{
\begin{array}{ccc}
f' & 7/2 & j' \\
 j & 1 & f 
 \end{array}
 \right\}
\end{equation}
are the relative oscillator strengths determined by the relevant Wigner-6J symbol.
