...
In terms of these propagating modes the quantized electric field operator, \erf{Eq::QuantizedElectricField}, becomes
	\begin{align} \label{Eq::PropagatingElectricField}
		...
		=& \sum_{b,p} \sqrt{ \frac{2 \pi \hbar \omega_0}{ v_g} } \big[ \mathbf{u}_{b,p}(r\!_\perp,\phi) \hat{a}_{b,p}(z,t)\nn\\ 
		&+ \mathbf{u}_{b,p}(r\!_\perp,\phi) \hat{a}_{b,p}(z,t) \big] e^{i \beta_0 z},
	\end{align}	
...
The propagating electric field, \erf{Eq::PropagatingElectricField}, interacts with the trapped atoms via the dispersive light-shift Hamiltonian~\cite{deutsch_quantum_2010,kien_dynamical_2013,baragiola_open_2014},
	...
	\end{equation}
where $\poltens^{(n)}$ is the atomic tensor polarizability operator, given in \erf{Eq::PolarizabilityOperator}, for the $n^{th}$ atom trapped near the nanofiber surface at position $\mathbf{r}'_n$.   
