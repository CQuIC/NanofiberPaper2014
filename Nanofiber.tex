...
\begin{minipage}{.32\linewidth}
\centering
\subfloat[]{\label{fig:kappa_q_D1_xyplane_NA2500_r2a_m44}\includegraphics[scale=0.4]{./Figs/kappa_q_D1_xyplane_NA2500_r2a_m44}}
\end{minipage}
...
We illustrate the effects of geometry and choice of quantization axis on the atom-light coupling in  \frf{fig:kappa_q_D1_xyplane_NA2500_r2a_m44}. 
...
In \frf{fig:xit_qXoptimal_r1d8a_NA2500} we investigate the dynamics of the squeezing parameter, $ \xi^{2}=N_A \expect{\Delta J_3^2}/\expect{J_1}^2 $, at the optimal quantization axis $\phi_q\approx 86.2^\circ$ given $ 2500 $ trapped at $ r'\!_\perp=1.8a $ on the \emph{x}-axis and compare to using \emph{x}-axis as the quantization axis which is the optimal choice of quantization axis for the idealized situation with no optical pumping where the dynamics results solely from the variations in $\kappa$ (see \frf{fig:kappa_q_D1_xyplane_NA2500_r2a_m44}) and squeezing continues indefinitely.
...
\end{minipage}
\caption{ Optimal choice of quantization axis considering optical pumping. \protect\subref{fig:phiq_optimal_NA1000to5000_rp}: Optimal quantization direction, $ \phi_q $, on the transverse plane as a function of atom distance to the core of the nanofiber, $ r`\!_\perp $, and atom number.
\protect\subref{fig:xi_optimal_NA1000to5000_omega44}: Peak squeezing parameter as a function of atom position and atom number when the optimal quantization axis is used. Subfigs.~\protect\subref{fig:domega_magic44_qXYoptimal_rp} and~\protect\subref{fig:domega_magic33_qXYoptimal_rp} show the magic detuning, $\Delta_{44}^m/2\pi$ and $ \Delta_{33}^m/2\pi $, as functions of $ r`\!_\perp $ for three choices of $\qaxis$, along the \emph{x}-axis and \emph{y}-axis and optimal quantization axis for squeezing.
...
However, in \frf{fig:phiq_optimal_NA1000to5000_rp} we see that when optical pumping is included, the optimal $\qaxis$ is rotated approaching to the \emph{y}-axis and the magic frequency, $ \Delta_{44}^m $, is detuned nearly equidistant from $F'=3$ and $F'=4$. 
...
Using the optimal quantization axis, the peak squeezing is plotted in \frf{fig:xi_optimal_NA1000to5000_omega44}.
...
Shown in \frf{fig:gamma34_q_D1_xyplane_NA2500_r1d8a_m44} is one important spin flip rate due to optical pumping from $ \ket{\uparrow} $ to $ \ket{\downarrow} $ which kicks the optimal quantization axis off the \emph{y}-axis for a smaller value of $ \gammaud $ or weaker decoherence.
...
\begin{minipage}{.32\linewidth}
\centering
\subfloat[]{\label{fig:gamma34_q_D1_xyplane_NA2500_r1d8a_m44}\includegraphics[scale=0.4]{./Figs/gamma34_q_D1_xyplane_NA2500_r1d8a_m44}}
\end{minipage}
\begin{minipage}{.32\linewidth}
\centering
...
\protect\subref{fig:gamma34_q_D1_xyplane_NA2500_r1d8a_m44}: The normalized $ \gammaud/\gamma_s $ as a function of $ \phi_q $ on the \emph{xy}-plane. 
...
As expected, this yields higher peak squeezing, \frf{fig:xi_optimal_NA1000to5000_omega44}. 
