...
\todo[inline]{Need to decide if we should put the detailed derivations to the appendix.}
We present a general theory on light-atom interaction in presence of a nanofiber in Appendix~\ref{sec:lightatominteract}. 
In this section, we only consider the atom number measurement technique with atomic states restricted in Clock states. 
...
They satisfy the white-noise commutation relations
	...
	\end{align}	
Note that the commutation relation is related to the \emph{phase retarded time} which is related to the phase velocity, $v_p = \omega_0/\beta_0$, rather than the group velocity~\cite{LeKien2008}.
...
The effective Hamiltonian of the light-atom interaction at time $ t $ can then be given by~\cite{Deutsch2010a}
\begin{align}  
	H_{\rm eff}   = - \hat{\mathbf{E}}^{(-)}(\mathbf{r}'& ; t ) \cdot \tensor{\alpha} \cdot \hat{\mathbf{E}}^{(+)}(\mathbf{r}';t )\\
	= -\frac{2\pi \hbar \omega_0}{ v_g } &\Big( \mathbf{u}^*_H(r^\prime\!_\perp, \phi') 
	\!\cdot\! 
...
Master equation and jump operators...

Squeezing parameters and solutions...
