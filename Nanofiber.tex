...
Strong coupling between atoms and photons %is a prerequisite for a quantum atom-light interface 
...
A particular system that combines the elements above consists of cold atoms trapped in the evanescent field of the guided mode of an optical nanofiber \cite{vetsch_optical_2010, lacroute_state-insensitive_2012, balykin_quantum_2014} (see Fig. 1).  
While the nanofiber, with enhancement factor of $\sigma_0/A \sim  10^{-2}$, boosts $\Gamma_{\oneD}$ by orders of magnitude over free space for paraxial beams, one cannot reach the strong coupling regime, where $\Gamma_{\oneD}$ is on the order of $\Gamma_{\vac}$, as is possible in some engineered nanophotonic waveguides such as those arising in photonic crystals \cite{hung_trapped_2013}, where atoms can be trapped at positions of peak intensity of the field.  
One can, however, achieve strong cooperativity in the dispersive regime with small ensembles.  
When compared to free space, all light scattered into the guided mode is automatically mode matched, and thus, given the large ratio $\sigma_0/A$, one can achieve \error{high OD} with only a few thousand atoms.  
Such strong cooperativity opens the door to new regimes to create non-Gaussian quantum states of the ensemble \cite{strobel_fisher_2014} and potentially to implement nonlinear optics at the level of a few photons \cite{shen_coherent_2005, oshea_fiber-optical_2013} .
...
In this paper we study the nanofiber-based quantum atom-light interface in the dispersive regime.  
We focus here on the spin-polarization coupling induced by the elastic scattering of tensor-polarizable cesium atoms trapped near the surface of the nanofiber.  
This provides an entangling interaction to induce spin squeezing of the cesium clock-states, via QND measurement.  
Our analysis unifies a variety of different approaches found in the literature, including direct calculation of the dyadic Greens function for photon scattering [] and the input-output formalism studied for one-dimensional field theories based on the Heisenberg-Langevin equations [].
...
%========= FIGURE: Geometry (coupling strength, magic wavelength, area and detuning) =========%
\begin{figure}[b]
\includegraphics[scale=1.0]{./Figs/Fig_ModeMatch}
\caption{Depiction of mode-matching for various atom-light geometries. (a) Single atom in a tightly focused beam. The atom experiences a large field, but its scattered light is poorly mode-matched to the probe. (b) Cold atomic cloud probed by a paraxial beam. Cooperatively scattered radiation interferes constructively in the forward direction. (c) Atoms trapped in 1D optical lattices near the surface of an optical nanofiber interacting with a fiber-guided probe. The field at each atom is large, and the constructive interference enhances scattering into the forward guided modes.}\label{Fig::ModeMatching}
\end{figure}
%=============================================
...
Second and more importantly, the tight spatial confinement, as measured by the ratio the scattering cross section to the mode area, significantly increases the coupling strength over free space for every atom along the nanofiber, which yields strong cooperativity.
...
For a Gaussian beam with beam waist $w_0$, the total phase shift induced by a collection of polarizable atoms will be $\delta \phi = N_{\eff} 2 \pi k_0\Re({\alpha})/A$, where $A = \pi w^2_0/2$ is the beam area at the focus and $N_{\eff}$ is the effective number of atoms that radiate into this mode.  
...
This trade-off is illustrated \frf{Fig::ModeMatching}.  
\error{In \frf{Fig::PhaseShift}(b), we compare the total phase shift and phase shift per atom (see Fig. 5.4 of my thesis) for the nanofiber and for a 3D Gaussian cloud in free space for varying probe waists. }
...
\hat{\tensor{\boldsymbol{\alpha}}} \ket{g} \big] \cdot \mathbf{u}_{+, p}(\br'_\perp) =& - \frac{ \comment{ \omega_{0} } }{ \hbar v_g} \sum_e \frac{2 \pi |\bra{e}\hat{\mathbf{d}}\ket{g} \cdot \mathbf{u}_{+,p}(\br'_\perp)|^2}{ \Delta_{eg} } ,
	\end{align}
...
We define coordinate axes $(x,y,z)$ with $z$ oriented along the fiber axis for forward propagation, and the $x-z$ plane is defined such that the two chains of atoms lie at on the $x$-axis at $\phi' = \{0, \pi\}$.
...
The propagating electric field, \erf{Eq::PropagatingElectricField}, interacts with the trapped atoms via the dispersive light-shift Hamiltonian~\cite{deutsch_quantum_2010,kien_dynamical_2013},
	...
	\end{equation}
where $\poltens(\br'_n)$ is the atomic tensor polarizability operator, given in \erf{Eq::PolarizabilityOperator}, for the $n^{th}$ atom trapped near the nanofiber surface at position $\mathbf{r}'_n$.   
...
The atomic couplings to the $\{H,V\}$ modes,
	...
	\end{align}
are formed by $\hat{\alpha}_{p p'} = \mathbf{e}^*_{p'}(\phi') \cdot \poltens \cdot \mathbf{e}_{p}(\phi') $, the quantum mechanical analog of the polarizability components that appear in \erf{Eq::PhaseShift}, weighted by the transverse mode functions at the atomic position.

We explore a QND measurement of ${}^{133}$Cs atoms in the electronic ground state, $6S_{1/2}$ via polarization spectroscopy based on the collective atom-light coupling described by the dispersive light-shift Hamiltonian in \erf{Eq::GenHamiltonian}. 
...
For detuning $\Delta$ large compared to the excited hyperfine splitting on the D2 line, the unit-oscillator scattering rate is $\gamma_s =(\sigma_0 /A_{\rm in})(\Gamma/2 \Delta)^2 \dot{N}_L $, with effective area determined by the probe at the atomic position, \erf{Eq::AreaIn}.  
In this limit, the rotation angle $\chiN = (2/3) (\sigma_0/\Abir)(\Gamma/4\Delta)$ [\erf{Eq::RotationAngle}], yields a shot noise-limited atom number resolution, 
...
Because the quantization axis of the atom can be chosen arbitrarily and the guided probe light at the position of the atom will generally be elliptical, the light-shift interaction coherently couples different magnetic sublevels in a given manifold $F$, and thus does conserve $J_3$.  
...
In the standard way, squeezing the uncertainty in $\jz$ by QND measurement can be generated by preparing the atoms in a SCS along $\jx$, passing a probe prepared along $\hat{S}_2$ with photon flux $\dot{N}_L$, and continuously monitoring the $S_3$-component of the guided light in a polarimeter. The measurement strength,
	...
	\end{align}
quantifies the rate at which conditional projection noise is reduced, with $\chieff$ given in \erf{eq:chiJ3}. 
...
Finally, in \frf{Fig::CouplingStrength}(d) we show the variation in $\chieff$ as a function of $\phi_\pi$.  
...
The treatment above considersan idealized QND interaction. 
...
In addition it reduces the metrologically useful signal.  
...
We fully model this from a first-principles stochastic master equation (SME)~\cite{jacobs_straightforward_2006, baragiola_three-dimensional_2014},
...
The first two terms describe the QND measurement with strength $\kappa$ given in \erf{Eq::MeasurementStrength} and $dW$ is a stochastic Weiner increment satisfying $dW^2 = dt$. 
...
Here, $\gamma_{F}$ is the total rate of photon scattering by atoms in the clock state $\ket{F,0}$, defined by
	...
	\end{equation}
The effective non-Hermitian light-shift Hamiltonian that accounts for the imaginary part of the atomic polarizability is found by substitution of $\charpol \rightarrow -\frac{\sigma_0}{4\pi k_0}\frac{\Gamma/2}{\Delta_{FF'}+i\Gamma/2}$ in \erf{Eq::LightShiftHam}.  The rates of optical pumping between the clock states, $\ket{F,0} \rightarrow \ket{\tilde{F},0}$, are
	\begin{equation}
...
Typical time evolution is shown in \frf{Fig::Squeezing_Dynamics} for $2500$ atoms trapped on the \emph{x}-axis at a distance $r'\!_\perp=1.8a$ from the center of the nanofiber. 
Dimensionless time is scaled to the characteristic scattering rate, $\gamma_s \equiv %\frac{\sigma( \Delta_\eff)}{A_{\rm in}} \dot{N}_L=
...
With $2500$ atoms at $r_\perp = 1.8a$, peak squeezing reaches 4.7 dB when the clock states are chosen along the optimal axis; $\qangle^{\rm opt} \approx 86^\circ$ in \erf{Eq::QuantizationAxis}. 
...
Both effects are captured by the depolarization rate $\gamma_{11}$ in the equation for $\expt{\jx}$, \erf{Eq::MeanSpinDecay}, whose solution is plotted in \frf{Fig::Squeezing_Dynamics}(b).
...
Since the constituent atoms cannot be individually squeezed within the spin-$\half$ pseudospin, the reduced fluctuations in $\varz$ result from spin-spin correlations between the atoms. 
The collective $J_3$-variance is a combination of single-body and two-body terms, which takes the form
	\begin{align}
		\varz & = N_A \expt{ (\Delta j_3^{(1)} )^2} + N_A(N_A-1) \expt{ \Delta j_3^{(1)} \Delta j_3^{(2)} }.
	\end{align}
for permutationally symmetric states considered here. 
Loss of atoms and decay of $\expt{ \jz}$ affect the single-body term, which scales as $N_A$. The two-body covariance,
	\begin{align}
		\expt{ \Delta j_3^{(1)} \Delta j_3^{(2)} } = \smallfrac{1}{4} \big( \expt{ \hat{\sigma}_3^{(1)} \otimes \hat{\sigma}_3^{(2)}  } - \expt{\hat{\sigma}_3^{(1)} }^2  \big)
	\end{align}
which scales as $N_A^2$, has a much larger influence on the total variance. 
The spin-spin correlations, $\expt{ \hat{\sigma}_3^{(1)} \otimes \hat{\sigma}_3^{(2)} }$, which are absent in the initial SCS, are generated by the measurement and form the foundation of QND spin squeezing.  
Optical pumping acts locally to decohere the spin-spin correlations according to Eqs. (\ref{Eq::TwoBodyDecay}--\ref{Eq::OperatorMap}),
...
The fundamental decoherence in \erf{Eq::TwoBodySpinDecay} directly competes with the measurement and limits the amount of entanglement generated between atoms in the ensemble.  
All of these effects are included in the equation for $\varz$, \erf{Eq::varJz}, whose dynamical evolution is shown in Fig. \ref{Fig::Squeezing_Dynamics}(c). 
...
We plot the peak squeezing for $\qaxis$ in the $x-y$ plane in \frf{Fig::Squeezing_QuantizationAxis}(a). 
...
First, the rate of squeezing is determined by the OD/atom $=\sigma_0 A_{\rm in}/A_{J_3}^2$ (or equivalently, the ratio of the measurement strength to the scattering rate $\kappa/\gamma_s$), which peaks when $\qaxis$ is along the $y$-axis, as seen in \frf{Fig::Squeezing_QuantizationAxis}(a). 
...
At distances \comment{$r_\perp > 1.5a$} the atoms' local environment is roughly that of unmodified vacuum \cite{le_kien_spontaneous_2005} and a free-space optical pumping model in \erf{Eq::OpticalPumpingMapSchr} suffices. 
...
We studied the strong cooperativity in the atom-light interface that can be achieved in an optical nanofiber platform in the dispersive regime.  
...
In contrast, for atoms trapped in free space, the atom-light coupling is typically orders of magnitude smaller, $ \mathrm{OD}/\mathrm{atom} \sim 10^{-6}$.  
...
As the light is entirely guided, fiber-couped atomic ensembles can be simply networked together for truly long-range entanglement generation and distribution or combined with other physical platforms for hybrid quantum interfaces \cite{hafezi_atomic_2012}. 
...
Even greater improvement might be achieved by combining the enhancements of the tight transverse confinement of guided modes with longitudinal confinement provided by fiber-based optical cavities~\cite{le_kien_intracavity_2009, wuttke_nanofiber_2012, yalla_cavity_2014, bohnet_reduced_2014,nayak_optical_2014, kato_strong_2015}.  
...
For our choice of initial SCS along $\expt{\jx}$, the metrologically relevant squeezing parameter in \erf{Eq::SqueezingParameter} is $\xi^2 = N_A \expt{\Delta \hat{J}_3^2}/\expt{\hat{J}_1}^2$.  
...
The equation of motion for the conditional variance, \erf{Eq::varJz}, then follows from Eqs. (\ref{Eq::J3MomentEquations}).
