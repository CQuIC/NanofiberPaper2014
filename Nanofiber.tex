...
The dispersive interaction between the atoms and nanofiber guided photons provides a mechanism with which to perform a QND measurement on the atoms. The Lipmann-Schwinger input-ouput scattering equation, Eq. (?) follows for the evolution generated by the light-shift Hamiltonian~\cite{Deutsch2010a}
\begin{equation}  
	H_{LS}   = - \sum_i \hat{\mathbf{E}}^{(-)}(\mathbf{r}'_i ; t ) \cdot \hat{\tensor{\boldsymbol{\alpha}}}^{(i)} \cdot \hat{\mathbf{E}}^{(+)}(\mathbf{r}'_i;t ),
\end{equation}
where $\hat{\tensor{\boldsymbol{\alpha}}}^{(i)}$ is the polarizability tensor operator, Eq. (?), for an atom trapped near the nanofiber surface at position $\mathbf{r}'_i$.  When restricted the single HE$_{11}$ guided mode propagating in the positive $z$-direction, with quasi-linear polarizations $H$ and $V$ , 
\begin{equation}
...
\end{equation}
where the creation and annihilation operators satisfy the usual commutation relations for free propagating modes in the interaction picture and equation positions along the fiber axis,
\begin{equation}
\left[\hat{a}_\mu(z,t),\hat{a}^\dag_\nu(z,t')\right]=\delta_{\mu,\nu}  \delta(t-t'),
\end{equation}
The Hamiltonian thus takes the form
\begin{equation}  
	H_{LS}   = -\frac{2 \pi \hbar \omega_0}{v_g} \sum_i \sum_{\mu,\nu} \hat{\alpha}^{(i)}_{\mu \nu}\; \hat{a}_{\mu}^\dag(z_i,t)  \hat{a}_{ \nu}(z_i,t)
\end{equation}
where
\begin{equation}  
\hat{\alpha}^{(i)}_{\mu \nu}= \mathbf{u}^*_\mu(r'_\perp,\phi')  \cdot \hat{\tensor{\boldsymbol{\alpha}}}^{(i)} \cdot \mathbf{u}_\nu(r'_\perp,\phi')
\end{equation}
We explore QND measurement of atoms pre
...
The light shift Hamiltonian, \erf{eq:LightShiftHam_CG}, can then be expressed in this basis by finding the matrix elements between our relevant states. 
