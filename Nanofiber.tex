...
\frac{g_{\mu, e,g'}}{\omega\!-\!\omega_{eg} \!+\! i \Gamma\!_{e}/2  }\, \hat{a}_\mu (t_0) \nn\\
	...
	\end{align}
By substituting this into \erf{Eq::aguidedEOM} and defining asymptotic modes, $\hat{a}^{\inp}(\omega) = \lim_{t_0\rightarrow -\infty} \hat{a}(t_0) e^{i\omega t_0}$, $\hat{a}^{\out}(\omega) = \lim_{t\rightarrow +\infty} \hat{a}(t) e^{i\omega t}$ \cite{fan_input-output_2010}, we obtain the input-output relationship for the guided modes,\comment{(from here till next session, $ \hat{a}^{\inp}_\mu(\omega) $ has been changed to $ \hat{a}^{\inp}_{b,p}(\omega) $ since $ \mu $ actually includes $ \omega $... )}
...
by noting, for the guided-mode dyadic Green's function given in \erf{Eq::GreensGuided},
	\begin{align}
		...
		%\! =\! - i \frac{2 \pi}{\hbar} \!\!\!\sum_{e,g,g'}\!\!  \hat{\sigma}\!_{gg'} \frac{g_{\!\mu,e,g}^* g\!_{\mu'\!\!,e,g'}}{ \Delta_{eg} \!\!+\! i \Gamma\!_{e}/2 }, 
	\end{align}
