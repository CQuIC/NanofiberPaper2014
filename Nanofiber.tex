...
\change{Optical pumping drives two processes that limit the amount of spin squeezing that can be produced.  The first is atom loss, which results from atoms being pumped out of the clock subspace into other magnetic sublevels \comment{(why do we not have to worry that these sublevels might affect the birefringence?)}. The second is spin flips, where at atom is pumped between the two states of the pseudospin.  Due to the ground hyperfine splitting of 9.2 GHz, spin flips are dominated by the processs $\ket{\uparrow} \rightarrow \ket{\downarrow}$.
