...
\newcommand{\zrateB}{\Gamma_{3}}
\newcommand{\drate}{\Gamma_{\rm dec}}
...
\frac{g_{\mu, e,g'}}{\omega-\omega_{eg} + i \Gamma_{e}/2  }\, \hat{a}_\mu (t_0) \\
	...
	\end{align}
\comment{In fact, this is obtained by defining $ \hat{\sigma}_{ge'}(t)=\int \frac{\mathrm{d}\omega}{\sqrt{2\pi}}\hat{\sigma}_{ge'}(\omega) e^{-i\omega(t-t_0)} $. Can we call it the steady state result for the above equation?}
...
\poltens \cdot \mathbf{u}_{\mu'} (\br') =  i \frac{ 2\pi \omega_0}{v_g} \mathbf{u}^*_\mu 
 ...
 \hat{\sigma}_{gg'} \frac{g_{\mu,e,g}^* g_{\mu',e,g'}}{ \Delta_{eg} + i \Gamma_{e}/2 }, 
	\end{equation}
...
We now focus on the forward-propagating guided modes $(f=+)$ and drop the $f$ index.  	The propagating electric field, \erf{Eq::PropagatingElectricField}, interacts with the trapped atoms via the dispersive light-shift Hamiltonian~\cite{deutsch_quantum_2010,kien_dynamical_2013},
	...
	\end{equation}
Here, $\poltens(\br_n)$ is the atomic tensor polarizability, given in \erf{Eq::PolarizabilityOperator}, for the $n^{th}$ atom trapped near the nanofiber surface at position $\mathbf{r}_i$. The output field modes are given by the Fourier transform of \erf{Eq::aout}, yielding \cite{le_kien_correlations_2008} \change{
...
The same birefringent interaction, \erf{Eq::GenHamiltonian}, can be utilized in a QND measurement to squeeze the projection noise of the collective atomic spin.  We consider squeezing of the uncertainty associated with the ``clock" transition of cesium, which to first order is insensitive to external magnetic fields.  The clock states, $\ket{\uparrow} = \ket{6S_{1/2}, F=4, M=0}$ and $\ket{\downarrow} = \ket{6S_{1/2}, F=3, M=0}$, define a pseudospin within each atom, $\hat{\sigma}_z \equiv \op{\uparrow}{\uparrow} - \op{\downarrow}{\downarrow}$.  The quantum uncertainty in the collective pseudospin,
	\begin{align}
		\hat{J}_z = \frac{1}{2} \sum_{n=1}^{N_A} \hat{\sigma}_z(\mbf{r}_n),  
	\end{align}
fundamentally limits the precision of atomic clocks \cite{wineland_spin_1992}. For atoms prepared in an unentangled SCS the projection noise, $\Delta J_z^2 \big|_{\scs} = N_A/4$, sets the standard quantum limit for spin measurements. A spin squeezed state (SSS) exhibits reduced fluctuations, $ \Delta J_z^2 \big|_{\rm SSS}  < N_A/4$, due to negative pairwise correlations between the atoms \cite{norris_enhanced_2012}. A SSS provides improved phase estimation over a SCS when the spin squeezing parameter $\xi^2$ as defined by Wineland \emph{et al.} \cite{wineland_spin_1992}, satisfies
	\begin{align} \label{Eq::SqueezingParameter}
		\xi^2 \equiv N_A \frac{ \Delta J_z^2 }{ \expt{\hat{J}_{||}}^2 } < 1.
	\end{align}
...
Within the clock-state subspace the rank-1 vector light shift in the dispersive Hamiltonian, \erf{Eq::GenHamiltonian}, vanishes since $\bra{\uparrow} \hat{F}_k \ket{\uparrow} =\bra{\downarrow} \hat{F}_k \ket{\downarrow} = 0$ for any choice of quantization axis. Further, as shown below, atoms on either side of the nanofiber experience the same birefringent coupling. The resulting entangling Hamiltonian arises from the combination of scalar and tensor light shifts \cite{chaudhury_continuous_2006},
	\begin{align} \label{Eq::ClockHamiltonian}
		\hat{H}_{LS} = \hbar \Big\{ %& \error{ \big[ \big( \chi_{H,\uparrow} +\chi_{V,\uparrow} \big) + \big( \chi_{H,\downarrow}+ \chi_{V,\downarrow}\big) \big] \frac{N_A}{2} \hat{S}_0(t) } \\
		 ...
		 & \big[ \big( \chi_{H,\uparrow} +\chi_{V,\uparrow} \big) - \big( \chi_{H,\downarrow} + \chi_{V,\downarrow}\big) \big] \hat{J}_z \hat{S}_0(t) \\
		+ & \big[  \big( \chi_{H, \uparrow} - \chi_{V,\uparrow} \big) - \big(\chi_{H,\downarrow} - \chi_{V,\downarrow} \big) \big]  \hat{J}_z \hat{S}_1(t) \Big\}. \nonumber
	\end{align}
...
where $F = \{4,3\}$ labels $\{\uparrow,\downarrow\}$. In the context of squeezing, the constant birefringence discussed above in the QND measurement of atom number can be canceled with a compensating waveplate. The first term in \erf{Eq::ClockHamiltonian} does not affect polarization spectroscopy, but will act to rotate the pseudo-spin around the $J_3$-axis of the generalized Bloch sphere.  While this does not affect the squeezing of projection noise in $\hat{J}_3$, it affects the metrologically relevant squeezing by adding uncertainty in the direction of the mean spin.  By choosing a ``magic wavelength" at which the light shifts on the two clock states are equal, 
	...
	\end{align}
this term can be cancelled \cite{chaudhury_continuous_2006}. Additional light shifts from the probe and trapping fields that can drive the atoms out of the clock subspace are mitigated with a bias magnetic field which shifts the auxiliary levels out of resonance \cite{smith_continuous_2004}. The level structure for probing on the D1-line of $^{133}$Cs is shown in \frf{Fig::}. \comment{(placeholder for a more detailed explanation). } 

The remaining QND interaction Hamiltonian is
	\begin{equation} \label{Eq::FaradayHam}
		\hat{H}_{LS} = \hbar \chi_{\eff} \hat{J}_z \hat{S}_1(t),
	\end{equation}
where the effective rotation angle at the magic wavelength,
$\chi_{\eff} = \big( \chi_{H, \uparrow} - \chi_{V,\uparrow} \big) - \big(\chi_{H,\downarrow} - \chi_{V,\downarrow} \big) = 2(\chi_{H, \uparrow}-\chi_{H, \downarrow})$, arises from the differential light shift produced by the $H$ mode.  For the generation of QND squeezing, each of the $N_A$ atoms is prepared in an equal superposition of $\ket{\uparrow}$ and $\ket{\downarrow}$ with respect to $\mbf{e}_\pi$, \emph{i.e.} a SCS along $\hat{J}_x$, $ \ket{\Psi_A} = \big(\frac{\ket{\uparrow}+\ket{\downarrow}}{\sqrt{2}}\big) ^{\otimes N_A}$. The interaction, \erf{Eq::FaradayHam}, maps the atomic projection noise onto the light, whose $S_3$-component is continuously monitored. A probe prepared along $\hat{S}_2$ with photon flux $\dot{N}_L$  implements a weak QND measurement of the collective atomic spin with measurement strength $\kappa$ that quantifies the rate at which conditional projection noise is reduced:
	\begin{align} \label{Eq::MeasurementStrength}
		\kappa \equiv |\chi_\eff|^2 \dot{N}_L, 
	\end{align}
with $\chi_\eff$ given in \erf{Eq::chieff}. \comment{Is this right? Where is the area of the input mode?}
...
with coefficients that depend on detunings and atomic structure,
	...
	\end{align}
Birefringence emerges from the anisotropy of the intensity of the $H$ and $V$ modes--the first term--and additionally from the tensor atomic response, which depends on $\mathbf{e}_\pi$.  \comment{(why aren't we showing these two separate contributions in a figure?)} Note that since the relative phase of the guided-mode $z$-component plays no role in \erf{Eq::ClockStateCoupling}, the coupling strength is identical for atoms above and below the fiber, \emph{i.e.} for $\phi' = \{0,\pi\}$.  We write the effective rotation angle in the Hamiltonian, \erf{Eq::FaradayHam}, as
...
The above treatment considers an idealized QND interaction. In an optical nanofiber geometry, the atoms are inevitably coupled to optical modes other than the forward guided mode, and photons scattered into unmeasured modes will cause decoherence of the collective spin through optical pumping. The maximum achievable squeezing is determined by the balance of this decoherence with the QND measurement. We can fully model this with a stochastic master equation (SME),
...
where $ \sigma (\Delta_{FF'} )  = \sigma_0 \Gamma^2/4\Delta^2_{FF'}$ is the the scattering cross section at the probe detuning.  When expressed in terms of Pauli operators on the pseudo-spin-1/2 of the clock states,
	\begin{align} \label{Eq::OpticalPumpingMapSchr}
		...
		=& - \bigg( \frac{2(\gammau+ \gammad) - \gammauu - \gammadd}{4} \bigg) \hat{\rho} - \frac{ \gammau - \gammad - \gammauu + \gammadd }{4} \big( \hat{\sigma}_z \hat{\rho} + \hat{\rho} \hat{\sigma}_z \big) \\
		...
		%& + \frac{\gammauu+\gammadd}{4} \hat{\sigma}_z \hat{\rho} \hat{\sigma}_z + \gammaud \; \hat{\sigma}_- \hat{\rho} \hat{\sigma}_+ + \gammadu \; \hat{\sigma}_+\hat{\rho} \hat{\sigma}_- .  \nn 
	\end{align} 
There are two important features of this map that are not typical in a QND measurement for spin-1/2 particles.  First, the map is {\em not} trace preserving as atoms are pumped out of the clock states. Second, unequal rates of optical pumping for $\ket{\uparrow}$ and $\ket{\downarrow}$ will polarize the mean $\expt{\hat{J}_z}$, different from the value found in a QND measurement.
...
Along with the QND measurement, these effects yield an equation for the squeezed conditional variance:
	\begin{align} \label{Eq::Jzvariance}
		\frac{d}{dt} \Delta J_z^2 & = - \kappa \big( \Delta J_z^2 \big)^2 - 2 \big( \lrate + \drate \big) \left( \Delta J_z^2 - \frac{N_A}{4} \right) - \lrate \frac{N_A}{4}. 
	\end{align}

Optical pumping has an additional effect on metrological squeezing, \erf{Eq::SqueezingParameter}, by shortening the macroscopic mean spin $ \expt{\hat{J}_x}$. This happens both through atom loss and through spin flips, $\ket{\uparrow_x} \rightarrow \ket{\downarrow_x}$, that occur at rate $\frate$ given in \erf{Eq::frate}. The mean spin thus decays exponentially according to
 	\begin{align}
		\frac{d}{dt} \expt{\hat{J}_x} =& - \big(\lrate + \frate ) \expt{\hat{J}_x}.
	\end{align}

We determine the metrological spin squeezing parameter $\xi^2$ by integrating the equations for $\Delta J_z^2$ and $\expt{\hat{J}_x}$ whose solutions are then inserted into \erf{Eq::SqueezingParameter}.  For no atom loss, $\lrate = 0$, the map in \erf{Eq::OpticalPumpingMapSchr} is trace-preserving, as would be the case for genuine spin-1/2 ground-state atoms such as $^{171}$Yb.  In this case \erf{Eq::Jzvariance} can be solved analytically \cite{baragiola_three-dimensional_2014}, and $\Delta J_z^2$ reaches a steady state where the QND squeezing equilibrates with the decoherence. Here, we solve the equation numerically. 

Figure \ref{Fig::CouplingStrength} proposes an optimal choice for $\qaxis$ along the $y$-axis, where $\chi_\eff$ reaches a maximum for a probe detuned nearly equidistant from $F'=3$ and $F'=4$. However, in \frf{Fig::Squeezing_QuantizationAxis}(a) we see that optical pumping rotates the optimal $\qaxis$ away from the \emph{y}-axis. Insight into the optimal balance between QND measurement and the optical pumping is gained by examining measurement strength and decay rates. The squeezing of $\Delta J_z^2$ is determined by the measurement strength, $\kappa/\gamma_s$, shown in \frf{Fig::Squeezing_QuantizationAxis}(b), where we define a characteristic photon scattering rate $\gamma_s = (\sigma_0/A_\eff)(\Gamma/2 \Delta_\eff)^2 \dot{N}_L$ that follows from \erf{Eq::chieff}. In the context of metrological squeezing, optical pumping affects the mean, $\expt{\hat{J}_x}$, and the variance, $\Delta J_z^2$, in different ways. The relative rates of atom loss, of decoherence in $\Delta J_z^2$, and of spin flips that shorten $\expt{\hat{J}_x}$ are shown in \frf{Fig::Squeezing_QuantizationAxis}(c). 
...
The adjoint form of the SME in \erf{Eq::SME} describes the evolution of the collective operators in the Heisenberg picture. For a collective operator single-body operator $\hat{X} = \sum_n \hat{x}^{(n)}$ the evolution due to optical pumping is $d\expt{ \hat{X}}|_{\rm op} = \sum_n \Tr [\mathcal{D}_n[\hat{\rho}]\hat{X} ] = \sum_n \Tr [\expt{\mathcal{D}_n\dg[\hat{x}^{(n)}]} ] $, where the map, which acts locally on atoms along the nanofiber, is given in \erf{Eq::OpticalPumpingMapSchr}. Applying the map to the single-atom operators, 
	\begin{align}
		\mathcal{D}\dg[\hat{\mathbbm{1}}] & = - \lrate \hat{\mathbbm{1}} - \prate \hat{\sigma}_z \label {Eq::Idecay} \\
		\mathcal{D}\dg[\hat{\sigma}_z] & = - \zrateA  \hat{\mathbbm{1}} - \big( \lrate + \drate \big) \hat{\sigma}_z \label{Eq::zdecay} \\
		\mathcal{D}\dg[\hat{\sigma}_x] & = - \big(\lrate + \frate \big) \hat{\sigma}_x 
	\end{align}
with rates	
	\begin{align}
		...
		\frate & =  \frac{\gammauu + \gammadd + \gammaud + \gammadu}{2}. \label{Eq::frate}
	\end{align}
Here $\Delta_\uparrow \equiv \gamma_\uparrow - \big(\gammauu + \gammadu \big)$ is the total rate of population change for the state $\ket{\uparrow}$ and similarly for $\Delta_\downarrow$. Atom loss occurs when population is pumped out of the clock states, $\Delta_\uparrow + \Delta_\downarrow \neq 0$, and the atom becomes incoherently polarizated when $\Delta_\uparrow \neq \Delta_\downarrow$.
\comment{(Notations like $ \Delta_{\uparrow/\downarrow} $ seem confusing. Maybe we also want to keep definitions like $ \tilde{\gamma}_\uparrow =\gamma_\uparrow - \gammadu $ with proper physics interpretation to give alternative simpler expressions for some of our equations? ).}
...
Consider a collective two-body operator $\hat{O} = \sum_{m, n \neq m} \hat{x}^{(m)} \hat{y}^{(n)}$. The two-body correlations that form the basis for spin squeezing decay according to \cite{baragiola_three-dimensional_2014}
	...
	\end{align}
where the superscript refers to the $m^{th}$ and $n^{th}$ atoms. The two-body correlations that appear in the collective spin $\hat{J}_z^2$ thus decay as
	\begin{align}
		\frac{d}{dt} \hat{\sigma}_z^{(m)} \otimes \hat{\sigma}_z^{(n)} \Big|_{\rm op} = - 2 \big( \lrate + \drate \big)  \hat{\sigma}_z^{(m)} \otimes \hat{\sigma}_z^{(n)} - \zrateA \big(\hat{\mathbbm{1}}^{(m)} \otimes \hat{\sigma}_z^{(n)} + \hat{\sigma}_z^{(m)} \otimes \hat{\mathbbm{1}}^{(n)}\big),
	\end{align}
where the map for optical pumping within the pseudospin of each atom is given by \erf{Eq::zdecay}.  The equations of motion for the moments of $\hat{J}_z$ are now found from the SME, \erf{Eq::SME}, 
	\begin{subequations} \label{Eq::J3MomentEquations}
	\begin{align} 
		d \expt{\hat{J}_z} =& \error{\pm} \sqrt{\kappa} \Delta J_z^2 \, dW - \big( \lrate + \drate \big) \expt{\hat{J}_z}dt - \smallfrac{1}{2} \zrateA N_A dt ,  \\
		d \expt{\hat{J}_z^2} %=& \error{\pm} 2\sqrt{\kappa} \expt{\hat{J}_z}\Delta J_3^2 \, dW -  2 \zrateB \expt{\hat{J}_z^2}dt - \big( \lrate - 2\zrateB + 4\zrateA\expt{\hat{J}_z}  \big) \smallfrac{N_A}{4} dt- \Big( \smallfrac{\prate}{2} - \zrateA \Big) \expt{\hat{J}_z} dt . \nn \\
		=& \error{\pm} 2\sqrt{\kappa} \expt{\hat{J}_z}\Delta J_3^2 \, dW -  2 \big( \lrate + \drate \big) \expt{\hat{J}_z^2}dt + \smallfrac{1}{4} \big( \lrate + 2\drate \big) N_A  \\
		&- \zrateA \expt{\hat{J}_z}  \big( N_A - 1 \big)dt -  \smallfrac{1}{2} \prate \expt{\hat{J}_z}. \nonumber 
	\end{align}
	\end{subequations}
The stochastic term in $d\expt{\hat{J}_z^2}$ was simplified using the relation $\expt{J_z^3} = 3\expt{\hat{J}_z^2}\expt{\hat{J}_z}- 2\expt{\hat{J}_z}^3$, which comes from Gaussian statistics \cite{jacobs_straightforward_2006}. Finally, the It\={o} calculus governing the stochastically evolving moments requires that differentials be taken to second order, and the evolution of the variance is given by $d \Delta J_z^2 = d \expt{\hat{J}_z^2} - 2 \expt{\hat{J}_z} d \expt{\hat{J}_z} - \big( d \expt{\hat{J}_z} \big)^2$. In the limit where $\expt{\hat{J}_z}\ll N_A$ the terms proportional to the stochastically fluctuating mean can be dropped, and the equation of motion for the conditional variance, given in \erf{Eq::Jzvariance}, follows from Eqs. (\ref{Eq::J3MomentEquations}).
